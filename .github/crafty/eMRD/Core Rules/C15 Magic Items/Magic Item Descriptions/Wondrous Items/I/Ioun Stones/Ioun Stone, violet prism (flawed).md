---
ImportedOn: "Thursday, February 16, 2023 6:10:23 PM"
Aliases:
  - "flawed violet prism ioun stone"
  - "Ioun stone (violet prism) (flawed)"
Category: "Wondrous Items"
Tags:
  - "Category/Wondrous-Items"
parent:
  - "Ioun Stones"
up:
  - "Ioun Stones"
prev:
  - "Ioun Stone, yellow prism (flawed)"
next:
  - "Ioun Stone, vibrant yellow prism"
RWtopicId: "Topic_13047"
---
# Ioun Stone, violet prism (flawed)
## Overview
**Aura** strong varied; **CL** 12th

**Slot** —

**Price** 4,550 gp

**Weight** —

**Description**

This crystalline stone always floats in the air and must be within 3 feet of its owner to be of any use. When a character first acquires this stone, she must hold it and then release it, whereupon it takes up a circling orbit 1d3 feet from her head. Thereafter, the stone must be grasped or netted to separate it from its owner. The owner may voluntarily seize and stow the stone (to keep it safe while she is sleeping, for example), but she loses the benefits of the stone during that time. Ioun stones have AC 24, 10 hit points, and hardness 5.

This ioun stone allows the user to shoot a blue ray (ranged touch attack) and the target is sent to another plane (Will negates). Once the effect has been discharged, the stone becomes gray and useless*,* while orbiting.

**Construction**

**Requirements** [[Craft Wondrous Item (Feat)|Craft Wondrous Item]], creator must be 12th level

**Cost** 2,275 gp

**Source:** True Arcane Story: Ioun Stones (Louis Porter Jr. Design), pg. 7


---
## Tags
#Publisher/Louis-Porter-Jr--Design #Sources/True-Arcane-Story--Ioun-Stones #Rule/OGL #Rule/3PP #Rule-System/Pathfinder #Special-Item-Type/Magic-Item #Magic-Item-Type/Wondrous-Item #Rule/Wondrous-Item #Magic-Item-Slot/None #Price-Band/4-001---8-000-gp

