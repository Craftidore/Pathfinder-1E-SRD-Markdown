---
ImportedOn: "Thursday, February 16, 2023 6:10:23 PM"
Aliases:
  - "Keen"
Category: "Weapon Special Abilities"
Suffix: "MagWpnSpcAb"
Tags:
  - "Category/Weapon-Special-Abilities"
parent:
  - "Magic Weapon Special Ability Descriptions"
up:
  - "Magic Weapon Special Ability Descriptions"
prev:
  - "Ki Focus (MagWpnSpcAb)"
next:
  - "Icy Burst (MagWpnSpcAb)"
RWtopicId: "Topic_7496"
---
# Keen (MagWpnSpcAb)
## Overview
This ability doubles the threat range of a weapon. Only piercing or slashing melee weapons can be *keen*. If you roll this property randomly for an inappropriate weapon, reroll. This benefit doesn’t stack with any other effect that expands the threat range of a weapon (such as the *[[Keen Edge (Spell)|keen edge]]* spell or the [[Improved Critical (Feat)|Improved Critical]] feat).

Moderate transmutation; CL 10th; [[Craft Magic Arms and Armor (Feat)|Craft Magic Arms and Armor]], *[[Keen Edge (Spell)|keen edge]]*; Price +1 bonus.

**Source:** PF Core Rulebook, pg. 471


---
## Tags
#Sources/PF-Core-Rulebook #Rule-System/Pathfinder #Rule/Core #Rule/Weapon #Rule/Special-Ability-Weapon #Publisher/Paizo #Rule/OGL

