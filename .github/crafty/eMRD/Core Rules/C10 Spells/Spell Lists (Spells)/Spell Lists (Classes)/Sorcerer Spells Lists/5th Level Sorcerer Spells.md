---
ImportedOn: "Thursday, February 16, 2023 6:10:23 PM"
Aliases:
  - "Sorcerer 5"
Category: "Sorcerer Spells Lists"
Tags:
  - "Category/Sorcerer-Spells-Lists"
parent:
  - "Sorcerer Spells Lists"
up:
  - "Sorcerer Spells Lists"
prev:
  - "4th Level Wizard Spells"
next:
  - "7th Level Sorcerer Spells"
RWtopicId: "Topic_19037"
---
# 5th Level Sorcerer Spells
## Overview
An **<sup>M</sup>** or **<sup>F</sup>** appearing at the end of a spell’s name in the spell lists denotes a spell with a material or focus component, respectively, that is not normally included in a spell component pouch. An **<sup>X</sup>** denotes a spell with an XP component paid by the caster.

**Order of Presentation:** In the spell lists and the spell descriptions that follow them, the spells are presented in alphabetical order by name except for those belonging to certain spell chains. When a spell’s name begins with “lesser,” “greater,” or “mass,” the spell description is alphabetized under the second word of the spell name instead.

**Hit Dice:** The term “Hit Dice” is used synonymously with “character levels” for effects that affect a number of Hit Dice of creatures. Creatures with Hit Dice only from their race, not from classes, have character levels equal to their Hit Dice.

**Caster Level:** A spell’s power often depends on caster level, which is defined as the caster’s class level for the purpose of casting a particular spell. A creature with no classes has a caster level equal to its Hit Dice unless otherwise specified. The word “level” in the spell lists that follow always refers to caster level.

**Creatures and Characters:** The words “creature” and “character” are used synonymously in the spell descriptions.

**<sup>F</sup>** This spell has a focus component not normally included in a spell component pouch.

**<sup>M</sup>** This spell has a material component not normally included in a spell component pouch.

**<sup>R</sup>** Spell requires a requisite religion or race. If religion, spellcaster must worship the listed deity to utilize the spell. If race, the spell might only target members of the listed race (the spell will say this if it does), but often are just the race's guarded secrets. Members of other races can learn to cast them with GM permission.

**<sup>T</sup>** In order to prepare any of these spells, the caster must spend an hour performing a ritual in which he beseeches Torag (or a member of his family) for the aid of one of his divine family members. For 24 hours after the ritual, the caster may prepare spells of the requested deity. The caster may only attune himself to one additional deity at a time.

**<sup>Y</sup>** This spell has a Mythic version.

## A
**A**

**[[Absorb Toxicity (Spell)|Absorb Toxicity]]****:** You become immune to diseases and toxins, absorb one, and then spread it to others.

**[[Acid Rain (Spell)|Acid Rain]]** **<sup>M</sup>:** 20-ft.-radius cylinder deals 7d6 acid damage.

**[[Acidic Spray (Spell)|Acidic Spray]]****:** 1d6/level acid damage plus 1 round of acid.

**[[Aeonian Lantern (Spell)|Aeonian Lantern]]****:** Magical nonmoving lantern inflicts a –2 penalty to foes’ attacks, saves, and checks.

**[[Aerial Dispatch (Spell)|Aerial Dispatch]]****:** Gain +30 to fly speed, +1 to manueverability, +1 dodge bonus on AC and Reflex saves.

**[[Airy Water (Spell)|Airy Water]]****:** Turn normal water into a breathable substance; negate underwater movement and melee attack penalties.

**[[Amplify Ley Field (Spell)|Amplify Ley Field]]****:** Enhances [[Ley Lines and Shadow Roads|ley lines]] in the area by one step.

**[[Animal Growth (Spell)|Animal Growth]]****:** One animal doubles in size.

**[[Animate Construct V (Spell)|Animate Construct V]]****:** Animated material or construct serves you.

**[[Animate Dead, Improved (Spell)|Animate Dead, Improved]] <sup>M</sup>:** Creates Small [[Zombie (Bestiary PF1)|zombies]] from body parts.

**[[Animate Dead IV (Spell)|Animate Dead IV]]****:** Reanimates a once-living creature to fight for you.

**[[Animate Legion (Spell)|Animate Legion]]** **<sup>M</sup>:** Creates skeletons or zombies.

**[[Animate Living Skeleton (Spell)|Animate Living Skeleton]]** **<sup>Y</sup>:** Deals 1d6 hp + 1 Con, and staggers and sickens 1 round.

**[[Anticold Sphere (Spell)|Anticold Sphere]]****:** Sphere hedges out cold creatures and protects you from cold.

**[[Antifire Sphere (Spell)|Antifire Sphere]]****:** Creatures within sphere gain immunity to fire damage.

**[[Arc of Lightning (Spell)|Arc of Lightning]]****:** Line of electricity between two creatures (1d6/level damage).

**[[Arcane Fusion (Spell)|Arcane Fusion]]****:** Cast two spells you know without expending additional slots.

**[[Arcane Seal, Greater (Spell)|Arcane Seal, Greater]]****:** Seals one portal until a condition is met. Add 20 to the normal DC to break open a door or portal affected by this spell.

**[[Assassin's Senses, Greater (Spell)|Assassin's Senses, Greater]]**: Increases caster's critical threat range and multiplier by one.

**[[Assay Spell Resistance (Spell)|Assay Spell Resistance]]****:** +10 bonus on caster level checks to defeat one creature’s spell resistance.

**[[Augmenting Wall (Spell)|Augmenting Wall]]****:** Create a wall of faintly shimmering elemental energy; this can be configured to allow ranged weapons to pass through it in one direction.

**[[Awaken Tome (Spell)|Awaken Tome]]:** Spellbook gains human intellect.

## B
**B**

**[[Baleful Polymorph (Spell)|Baleful Polymorph]]****:** Turns subject into harmless animal.

**[[Baneful Casting, Greater (Spell)|Baneful Casting, Greater]]****:** Victim suffers 1d6/level of spell with backlash damage that may disrupt spellcasting.

**[[Baneweapon (Spell)|Baneweapon]]****:** Grants [[Bane (MagWpnSpcAb)|bane]] to one melee weapon or six projectiles.

**[[Banish Dragonmark (Spell)|Banish Dragonmark]]****:** Target loses dragonmark for 1 day/level.

**[[Barbed Defense, Greater (Spell)|Barbed Defense, Greater]]****:** Thorns deal 1d4 points of damage +1 per caster level and last for 1 round per caster level.

**[[Battle Ready, Greater (Spell)|Battle Ready, Greater]]****:** Store spells in a gem for use in quick response.

**[[Beast Shape III (Spell)|Beast Shape III]]**: You take the form of a Diminutive or Huge animal, or Small or Medium magical beast.

**[[Billet (Spell)|Billet]]****:** Creates sturdy barracks.

**[[Bind Guardian (Spell)|Bind Guardian]]****:** Bind a soul to animate a construct.

**[[Binding Oath (Spell)|Binding Oath]]****:** You seal an agreement on pain of a curse between two willing creatures.

**[[Bite of the Wereboar (Spell)|Bite of the Wereboar]]****:** You gain the Strength and attacks of a wereboar.

**[[Blackwater Tentacle (Spell)|Blackwater Tentacle]]****:** Create blackwater tentacle that attacks your foe.

**[[Blight (Spell)|Blight]]****:** Withers one plant or deals 1d6/level damage to plant creature.

**[[Blink, Greater (Spell)|Blink, Greater]]****:** Controlled blinking between the Material and Ethereal Planes grants defenses for 1 round/level.

**[[Blood Armor, Greater (Spell)|Blood Armor, Greater]]****:** As *[[Blood Armor (Spell)|blood armor]]*, but the increase to your AC is greater and lasts longer.

**[[Blood Fog (Spell)|Blood Fog]]****:** Subjects in fog get –10 to Str and Fortitude checks.

**[[Blood Tentacles]]****:** Create writhing tentacles from your chest.

**[[Bloody Claws (Spell)|Bloody Claws]]****:** Causes bleed damage with natural attacks.

**[[Bone Shards, Greater (Spell)|Bone Shards, Greater]]:** You fire shards that deal 1d6 hp/caster level damage + 1d4 hp [[S3.1A-C - Universal Monster Rules A-C (Bestiary 6 Apx)|bleed]].

**[[Bones of the Earth (Spell)|Bones of the Earth]]:** Creates pit that can be collapsed.

**[[Boreal Wind (Spell)|Boreal Wind]]****:** Gust of cold wind deals 1d4 cold damage/level and knocks creatures back.

**[[Brain Burrow Other (Spell)|Brain Burrow Other]]****:** Target creature occupies target corpse.

**[[Break Enchantment (Spell)|Break Enchantment]]****:** Frees subjects from enchantments, transmutations, and curses.

**[[Breath Weapon Substitution (Spell)|Breath Weapon Substitution]]****:** Your breath weapon deals a different kind of damage than normal.

## C
**C**

**[[Cacophonic Burst (Spell)|Cacophonic Burst]]****:** Noise deals 1d6/level sonic damage to all within area.

**[[Cacophonic Shield (Spell)|Cacophonic Shield]]****:** Shield 10 ft. from you blocks sounds, deals 1d6 sonic damage +1/level, and deafens creatures passing through.

**[[Call Dretch Horde (Spell)|Call Dretch Horde]]****:** Summons 2d4 dretches.

**[[Call Faithful Servants (Spell)|Call Faithful Servants]]****:** Summons 1d4 lantern archons, coure eladrins, or musteval guardinals.

**[[Call Lemure Horde (Spell)|Call Lemure Horde]]****:** Summons 2d4 lemures.

**[[Call Nightmare (Spell)|Call Nightmare]]****:** Summons a nightmare.

**[[Call of the Grave (Spell)|Call of the Grave]]****:** Target creature views you as an ally and obeys you.

**[[Call Zelekhut (Spell)|Call Zelekhut]]** **<sup>X</sup>:** A zelekhut performs one duty for you.

**[[Calm Air (Spell)|Calm Air]]****:** Reduce the strength of wind in an area.

**[[Chain of Bones (Spell)|Chain of Bones]]****:** Bone chains impede and can attempt to [[Grapple|grapple]], [[Disarm|disarm]], or [[Trip|trip]] creatures nearby.

**[[Chain of Shadows, Greater (Spell)|Chain of Shadows, Greater]]****:** Mingles targets’ innate resistances.

**[[Channeled Lifetheft (Spell)|Channeled Lifetheft]]****:** Fatigue, exhaust, and deal damage to target, depending on casting time.

**[[Channeled Sound Blast (Spell)|Channeled Sound Blast]]****:** Deal sonic damage in cone; area and damage increase based on casting time.

**[[Chaotic Spell Lash (Spell)|Chaotic Spell Lash]]****:** Magical whip inflicts a random status condition each time it strikes an enemy.

**[[Charm of Secrets (Spell)|Charm of Secrets]]****:** Target takes 10 hp/level if they divulge a specific secret.

**[[Choking Sands (Spell)|Choking Sands]]** **<sup>M</sup>:** Touched creature begins to suffocate on sand.

**[[Circlet of Enervation (Spell)|Circlet of Enervation]]****:** Subject takes damage and gains one negative level.

**[[Claws of the Earth Dragon (Spell)|Claws of the Earth Dragon]]****:** Damages a target creature and knocks them [[Prone|prone]].

**[[Cloak Ship (Spell)|Cloak Ship]]****:** As *[[Invisibility (Spell)|invisibility]]**,* except it affects an entire ship and its occupants.

**[[Cloudkill (Spell)|Cloudkill]]****:** Kills 3 HD or less; 4–6 HD save or die, 6+ HD take Con damage.

**[[Coat of Arms (Spell)|Coat of Arms]]****:** Surround yourself with blades of force.

**[[Combat Fluidness, Greater (Spell)|Combat Fluidness, Greater]]**: Caster gains +3 initiative bonus and +3 arcane bonus to AC, 10 minutes/level.

**[[Comfortable Cabin (Spell)|Comfortable Cabin]]****:** Cause a structure to become immune to natural environmental effects, while remaining comfortable inside.

**[[Commune with Texts (Spell)|Commune with Texts]]****:** Quickly absorb the knowledge in the target, which can be any assortment of books, scrolls, or other texts.

**[[Compel Tongue, Mass (Spell)|Compel Tongue, Mass]]****:** As *[[Compel Tongue (Spell)|compel tongue]]*, except it affects multiple creatures.

**[[Cone of Cold (Spell)|Cone of Cold]]****:** 1d6/level cold damage.

**[[Construct Body (Spell)|Construct Body]] <sup>Y</sup>:** Temporarily transforms you into a clockwork construct.

**[[Construct Essence (Spell)|Construct Essence]]****:** As *[[Construct Essence, Lesser (Spell)|lesser construct essence]]*, but grants more qualities of the construct type.

**[[Contact Other Plane (Spell)|Contact Other Plane]]****:** Lets you ask question of extraplanar entity.

**[[Contingent Energy Resistance (Spell)|Contingent Energy Resistance]]****:** Energy damage triggers a *[[Resist Energy (Spell)|resist energy]]* spell.

**[[Corpsewall (Spell)|Corpsewall]]:** Creates a wall of corpses that can be animated to attack and [[Grapple|grapple]].

**[[Create Chosen One (Spell)|Create Chosen One]]****:** Turn a normal human (of up to 3rd level) into a maddened, murderous creature.

**[[Create Soul Gem, Greater (Spell)|Create Soul Gem, Greater]]****:** As *[[Create Soul Gem (Spell)|create soul gem]]**,* but more time is available to capture the life force of the dying creature.

**[[Cryptwarden's Grasp (Spell)|Cryptwarden's Grasp]]****:** Bestow mummy rot with your touch.

**[[Crystal Shield (Spell)|Crystal Shield]]****:** Enhance your Shield of the Seraphim, adding +2 AC to the bonus that it provides.

**[[Cursed Blade (Spell)|Cursed Blade]]****:** Wounds dealt by weapon can’t be healed without *[[Remove Curse (Spell)|remove curse]]**.*

**[[Cyclonic Blast (Spell)|Cyclonic Blast]]****:** Deals 1d6 damage/level, knocks down creatures.

## D
**D**

**[[Dagger Swarm, Greater (Spell)|Dagger Swarm, Greater]]****:** Enchant a swarm of daggers to attack your foes, swarm can flank and sneak attack.

**[[Damnation Stride]] <sup>RY</sup>:** As *[[Dimension Door (Spell)|dimension door]]*, except you leave behind a burst of flame.

**[[Dancing Weapon (Spell)|Dancing Weapon]]****:** A weapon attacks on its own.

**[[Darkvision, Superior (Spell)|Darkvision, Superior]]****:** Gain darkvision with no range limitation.

**[[Dead Light (Spell)|Dead Light]]****:** Create a mote of light that illuminates a 30 feet radius but which is only visible by arcane means 

     such as *[[Arcane Eye (Spell)|arcane eye]]**,* *[[Arcane Sight (Spell)|arcane sight]]**,* *[[Arcane Sight, Greater (Spell)|greater arcane sight]]*, or *[[True Seeing (Spell)|true seeing]]*.

**[[Death From Above (Spell)|Death From Above]]****:** Target gains +30 enhancement bonus to [[Acrobatics (Skill)|Acrobatics]] and land speed, can leap and take full attack.

**[[Death Is Me (Spell)|Death is Me]]****:** Exchange features with a dead creature.

**[[Death Throes (Spell)|Death Throes]]****:** Your body explodes when you die.

**[[Deceptive Redundancy]]:** Protect your magic from attempts to dispel it.

**[[Defending Bone, Greater (Spell)|Defending Bone, Greater]]****:** As *[[Defending Bone (Spell)|defending bone]]**,* but with longer duration.

**[[Determine Causality (Spell)|Determine Causality]]** **<sup>F</sup>:** Influence the outcome of a random event favorably.

**[[Dimension Door, Greater (Spell)|Dimension Door, Greater]]****:** Short-range, multiple-use *[[Dimension Door (Spell)|dimension door]]*.

**[[Dimension Jumper (Spell)|Dimension Jumper]]****:** Teleport yourself up to 30 feet once per round.

**[[Dimension Shuffle (Spell)|Dimension Shuffle]]****:** Teleport multiple creatures short distances within line of sight.

**[[Dismissal (Spell)|Dismissal]]****:** Forces a creature to return to its native plane.

**[[Dispel Possession (Spell)|Dispel Possession]]****:** Forces possessing creature out of its host body.

**[[Dispel Water (Spell)|Dispel Water]]****:** Cancels water spells and effects or dismisses water creatures.

**[[Dispelling Breath (Spell)|Dispelling Breath]]****:** Your breath weapon acts as a targeted *[[Dispel Magic (Spell)|dispel magic]]* to all creatures in its area.

**[[Dominate Living Construct (Spell)|Dominate Living Construct]]****:** As *[[Dominate Person (Spell)|dominate person]]*, but affecting a living construct.

**[[Dominate Person (Spell)|Dominate Person]]****:** Controls humanoid telepathically.

**[[Draconic Might (Spell)|Draconic Might]]****:** Gain +5 to Str, Con, Cha; +4 natural armor; immunity to magic sleep effects and paralysis.

**[[Draconic Suppression (Spell)|Draconic Suppression]]****:** A dragon that fails its saving throw cannot use its breath weapon for the duration of the spell 

     and is treated as one age category younger for determining which special abilities it can use.

**[[Dragon Ally, Lesser (Spell)|Dragon Ally, Lesser]]** **<sup>X</sup>:** Exchange services with a 9 HD dragon.

**[[Dragonblood Spell-Pact (Spell)|Dragonblood Spell-Pact]]****:** Allows willing dragonblood creatures to trade spells.

**[[Dragonmark Demesne (Spell)|Dragonmark Demesne]]****:** Creates hemispherical shelter for you and 25 creatures.

**[[Dragonmarked Weapon, Greater]]****:** Weapon deals +2d6 damage to dragonmarked foes, hardness is doubled.

**[[Dragonsight (Spell)|Dragonsight]]****:** Gain low-light vision, darkvision, and blindsense.

**[[Dream (Domain)|Dream]]****:** Sends message to anyone sleeping.

**[[Dridershape (Spell)|Dridershape]]****:** You take on the form and abilities of a drider.

**[[Duelward (Spell)|Duelward]]****:** +4 on [[Spellcraft (Skill)|Spellcraft]] checks, counterspell as an immediate action.

**[[Dungeonsight (Spell)|Dungeonsight]]****:** You receive a clear and memorable view of the layout of chambers, barriers, and connecting passages within the area of effect.

## E
**E**

**[[Earth Reaver (Spell)|Earth Reaver]]****:** Eruption deals 7d6 damage to all in area.

**[[Elemental Body II (Spell)|Elemental Body II]]**: Turns you into a Medium elemental.

**[[Embed Ioun Stone (Spell)|Embed Ioun Stone]]****:** Take a single ioun stone and implant it into your body.

**[[Emerald Burst (Spell)|Emerald Burst]]** **<sup>M</sup>:** 20-ft.-radius burst dazes neutral creatures for 1 round, stuns evil creatures for 1d4 rounds.

**[[Energized Blood (Spell)|Energized Blood]]****:** Your blood deals energy damage to nearby creatures when you are struck.

**[[Enlarge Person, Greater (Spell)|Enlarge Person, Greater]]:** Subject remains enlarged for 1 hour/level.

**[[Eon of Sleep (Spell)|Eon of Sleep]]****:** Places the victim in a sleeping state for years.

**[[Ethereal Breath (Spell)|Ethereal Breath]]:** Your breath weapon manifests on the [[Ethereal Plane]].

**[[Etherealness, Swift (Spell)|Etherealness, Swift]]****:** Subject momentarily becomes ethereal.

**[[Evacuation Rune (Spell)|Evacuation Rune]]****:** Create invisible rune that you can teleport to as swift action.

## F
**F**

**[[Fabricate (Spell)|Fabricate]]** **<sup>M</sup>:** Transforms raw materials into finished items.

**[[False Gravity, Greater (Spell)|False Gravity, Greater]]****:** Target can travel on any solid surface as though that surface possessed its own gravity.

**[[False Vision (Spell)|False Vision]]** **<sup>M</sup>:** Fools scrying with an illusion.

**[[Feast on Fear (Spell)|Feast on Fear]]****:** Targets are [[Panicked|panicked]], and you gain temporary hit points.

**[[Feeblemind (Spell)|Feeblemind]]****:** Subject’s Int and Cha drop to 1.

**[[Fell Curse (Spell)|Fell Curse]]****:** Target suffers a ‐5 penalty on their next Will save after casting *[[Identify (Spell)|identify]]**.*

**[[Fetch II (Spell)|Fetch II]]****:** As *[[Summon Monster II (Spell)|summon monster II]]*, except called creatures do not disappear at the end of the spell’s duration.

**[[Fever Dream (Spell)|Fever Dream]]****:** Illusory vision fatigues or exhausts target, makes spellcasting difficult.

**[[Field of Resistance (Spell)|Field of Resistance]]****:** Zone provides SR 11 + caster level.

**[[Fiendform (Spell)|Fiendform]]****:** Assume form and abilities of fiendish creature, demon, or devil.

**[[Fiery Tentacles (Spell)|Fiery Tentacles]]****:** Like *[[Black Tentacles (Spell)|black tentacles]]*, but creatures in the area also take fire damage.

**[[Fire and Brimstone (Spell)|Fire and Brimstone]]****:** Subject takes fire damage, might be [[Sickened|sickened]].

**[[Fire Breath, Greater (Spell)|Fire Breath, Greater]]****:** You can breathe fire once/round; flame deals 1d8/two levels (maximum 10d8) damage to one target within 15 ft.

**[[Fire Fountain (Spell)|Fire Fountain]] <sup>M</sup>:** One 5-ft. burst/level deals 1d6 fire/level plus burning for 1 round.

**[[Fire Shield, Mass (Spell)|Fire Shield, Mass]]****:** Creatures attacking allies take damage; allies are protected from fire or cold.

**[[Fire Snake (Spell)|Fire Snake]]:** Creates a serpentine path of fire 5 ft. long/level that deals 1d6 fire damage/level.

**[[Fireburst, Greater (Spell)|Fireburst, Greater]]****:** Subjects within 15 ft. take 1d10/level fire damage.

**[[Firefont (Spell)|Firefont]]** **<sup>M</sup>:** One 5-ft. burst/level deals 1d6 fire/level plus burning for 1 round.

**[[Flank Shield, Greater (Spell)|Flank Shield, Greater]]****:** Target cannot be flanked.

**[[Flaying Tendrils (Spell)|Flaying Tendrils]]****:** You grow mind flayer tentacles, which you can use to attack your foes.

**[[Flaywind Burst (Spell)|Flaywind Burst]]****:** Cone blows away and knocks down smaller creatures and deals 1d6 damage/level.

**[[Flesh to Ice (Spell)|Flesh to Ice]]****:** Turns subject into statue of ice.

**[[Flesh to Salt (Spell)|Flesh to Salt]]****:** Turns creature into a statue of salt.

**[[Flying Carpet (Spell)|Flying Carpet]]****:** Targeted object gains the power to fly at the rate of 180 ft. with perfect maneuverability.

**[[Flowsight (Spell)|Flowsight]]****:** You can scry creatures in contact with a body of water.

**[[Force Lash, Greater (Spell)|Force Lash, Greater]]****:** Creates a whip of energy that inflicts 1d6 points of damage per level (max. 20d6).

**[[Form of the Desert Hunter (Spell)|Form of the Desert Hunter]]****:** Take the form of a dragonne.

**[[Form of the Threefold Beast (Spell)|Form of the Threefold Beast]]****:** You take the form of a chimera.

**[[Frequent Jaunt (Spell)|Frequent Jaunt]]****:** Short-range, multiple-use *[[Dimension Door (Spell)|dimension door]]*.

**[[Friend to Foe (Spell)|Friend to Foe]]****:** Make subject creature believe its allies are its enemies.

## G
**G**

**[[Gelid Blood (Spell)|Gelid Blood]]****:** Subject takes –4 penalty on attacks, AC, and Str- and Dex-related checks, 50% failure of spells with somatic components.

**[[Geniekind (Spell)|Geniekind]]:** Gain your choice of four different genie-themed appearances and powers.

**[[Geyser (Spell)|Geyser]]****:** Creates a geyser of boiling water.

**[[Ghost-Ash Wagon (Spell)|Ghost-Ash Wagon]]:** Create a haunted wagon, which is placed among the other vehicles of a caravan to protect them.

**[[Ghost Storm (Spell)|Ghost Storm]]****:** Targets failing a Fort save in the affected area lose any Dex bonus to AC, may only take a single action per round 

     and are subject to 1 negative level/round while they remain in the affected area.

**[[Glimpse of Eternity (Spell)|Glimpse of Eternity]]****:** Target takes 1d6 nonlethal damage/level and is [[Confused|confused]].

**[[Graymantle (Spell)|Graymantle]]****:** Inhibits creature’s ability to heal and regenerate for 1 round/level.

**[[Guide Craft, Greater (Spell)|Guide Craft, Greater]]****:** As *[[Guide Craft (Spell)|guide craft]]*, but with greater control.

**[[Gutsnake (Spell)|Gutsnake]]****:** 10-ft. tentacle grows from your stomach and attacks your enemies.

## H
**H**

**[[Half-Blood Extraction (Spell)|Half-Blood Extraction]] <sup>FMR</sup>:** Transform a [[Half-Orc|half-orc]] into a full-blooded [[Orc (Bestiary PF1)|orc]].

**[[Hammer of Mending (Spell)|Hammer of Mending]]****:** Wave of energy repairs items within its burst.

**[[Haunt Shift (Spell)|Haunt Shift]]****:** Turn corporeal and incorporeal undead into haunting presences.

**[[Heart of Fire (Spell)|Heart of Fire]]****:** Gain +10 ft. to land speed, resistance to fire 10, use *[[Fire Shield (Spell)|fire shield]]* once.

**[[Heart Ripper (Spell)|Heart Ripper]]****:** Kills living creature with up to your caster level in HD.

**[[Hidden Lodge (Spell)|Hidden Lodge]]****:** Creates sturdy cottage camouflaged to blend into natural surroundings.

**[[Hide from Dragons (Spell)|Hide from Dragons]]****:** Dragons can’t sense you.

**[[Hold Monster (Spell)|Hold Monster]]****:** As *[[Hold Person (Spell)|hold person]]*, but any creature.

**[[Hostile Juxtaposition (Spell)|Hostile Juxtaposition]]****:** You create a dimensional link with a targeted creature, and switch spots with it.

**[[Hungry Pit (Spell)|Hungry Pit]]:** As *[[Create Pit (Spell)|create pit]]*, but dealing 4d6 damage to those in it as it closes.

**[[Hunter's Mercy, Greater (Spell)|Hunter's Mercy, Greater]]****:** As *[[Hunter's Mercy (Spell)|hunter's mercy]]*, except lasts 1 round/level.

## I
**I**

**[[Ice Shape (Spell)|Ice Shape]]****:** Sculpts ice into any shape.

**[[Ice to Flesh (Spell)|Ice to Flesh]]****:** Restores frozen creature.

**[[Illusory Feast (Spell)|Illusory Feast]]****:** Subjects become [[Dazed|dazed]] by illusory food.

**[[Implacable Pursuer (Spell)|Implacable Pursuer]]:** You know where prey is, as long as it’s moving.

**[[Imprison Possessor (Spell)|Imprison Possessor]]****:** Subject that can possess creatures is trapped in current body.

**[[Incite Riot (Spell)|Incite Riot]]****:** Subjects attack nearest creature.

**[[Indomitability (Spell)|Indomitability]]:** Subject can’t be reduced below 1 hp.

**[[Insidious Suggestion (Spell)|Insidious Suggestion]]****:** Suggestion repeats over and over in creature’s mind.

**[[Interposing Hand (Spell)|Interposing Hand]]****:** Hand provides cover against 1 opponent.

**[[Invest Construct (Spell)|Invest Construct]]****:** Transfer your consciousness into a construct that fails a Will save.

**[[Investiture of the Narzugon (Spell)|Investiture of the Narzugon]]****:** Target gains a paralyzing gaze arrack.

**[[Investiture of the Orthon (Spell)|Investiture of the Orthon]]****:** Target disrupts extradimensional movement within 20 feet and automatically deals sonic damage to attackers.

**[[Involuntary Shapeshifting (Spell)|Involuntary Shapeshifting]]****:** If able to do so, target creature must change shape.

**[[Ioun Circuit (Spell)|Ioun Circuit]]****:** Create an invisible bond that stretches from an implanted stone to the center of your chest; resulting in a resonant power.

**[[Ironguard, Lesser (Spell)|Ironguard, Lesser]]****:** Subject becomes immune to nonmagical metal.

## J
**J**

**[[Jumpgout (Spell)|Jumpgout]]:** You create a vertical pillar of roaring flame.

## K
**K**

**[[Khain's Army (Spell)|Khain’s Army]]:** 1d4+1 [[Ghoul (Bestiary PF1)|ghouls]] and 1 [[Ghoul (Bestiary PF1)|ghast]] fight for you and explode when killed to deal damage.

**[[Killer's Eye, Greater (Spell)|Killer's Eye, Greater]]:** Gain +1d6 sneak attack damage/3 caster levels for 1 round/level.

## L
**L**

**[[Life Bubble (Spell)|Life Bubble]]:** Protects from environmental effects.

**[[Lightning Leap (Spell)|Lightning Leap]]****:** Transform into bolt of lightning, damaging foes and transporting yourself.

**[[Lightning Sphere (Spell)|Lightning Sphere]]****:** Energy ball deals 1d6/level electricity damage.

**[[Lucent Lance (Spell)|Lucent Lance]]:** Ambient light forms lance, deals various damage.

**[[Luminous Assassin (Spell)|Luminous Assassin]]****:** As *[[Luminous Assassin, Lesser (Spell)|lesser luminous assassin]]*, but the assassin is more powerful.

## M
**M**

**[[Mage's Faithful Hound (Spell)|Mage’s Faithful Hound]]****:** Phantom dog can guard a location and attack intruders.

**[[Mage's Private Sanctum (Spell)|Mage’s Private Sanctum]]****:** Prevents anyone from viewing or scrying an area for 24 hours.

**[[Magic Fang, Superior (Spell)|Magic Fang, Superior]]****:** Your natural weapons gain +1 enhancement bonus per four levels.

**[[Magic Jar (Spell)|Magic Jar]]** **<sup>F</sup>:** Enables possession of another creature.

**[[Magic Weapon, Mass Greater (Spell)|Magic Weapon, Mass Greater]]****:** Allies’ weapons gain +2 or greater enhancement bonus.

**[[Magical Backlash, Greater (Spell)|Magical Backlash, Greater]]****:** As *[[Magical Backlash (Spell)|magical backlash]]**,* except the target takes damage for 1 round/caster level.

**[[Mailed Might of the Magelords (Spell)|Mailed Might of the Magelords]]****:** Field of force around subject grants +8 armor bonus, immunity to 2nd- level or lower force spells, and DR 5/magic. 

**[[Major Creation (Spell)|Major Creation]]****:** As *[[Minor Creation (Spell)|minor creation]]*, plus stone and metal.

**[[Mana Flux (Spell)|Mana Flux]]****:** Magic in area has 20% failure chance.

**[[Manifest Dragon Heritage (Spell)|Manifest Dragon Heritage]]****:** Conjures a wyvern.

**[[Metal Skin (Spell)|Metal Skin]]****:** Grants +8 natural armor bonus, –2 to Dex.

**[[Metallic Wings, Lesser (Spell)|Metallic Wings, Lesser]]****:** Subject grows metallic wings which grant the ability to fly, a shield bonus to AC, and natural attacks.

**[[Miasma of Entropy (Spell)|Miasma of Entropy]]****:** Rot all natural materials in 30-ft. cone-shaped burst.

**[[Mind Fog (Spell)|Mind Fog]]****:** Subjects in fog get –10 to Wis and Will checks.

**[[Mirage Arcana (Spell)|Mirage Arcana]]****:** As *[[Hallucinatory Terrain (Spell)|hallucinatory terrain]]*, plus structures.

**[[Mirror of All Worlds (Spell)|Mirror of All Worlds]]:** Creates a mirror that can gaze into any world or plane.

**[[Moonbow (Spell)|Moonbow]]:** Three motes of electricity each deal 1d6/2 levels electricity damage to subjects.

**[[Mystic Shield, Lesser (Spell)|Mystic Shield, Lesser]]****:** Create an invisible aura around the target that moves with them and prevents all spells 

     and spell-like abilities of 3rd level or lower from affecting them.

## N
**N**

**[[Necrotic Burst (Spell)|Necrotic Burst]] <sup>F</sup>:** Encysted subject killed, cyst begins to roam.

**[[Necrotic Skull Bomb (Spell)|Necrotic Skull Bomb]]:** Exploding skull releases negative energy; targets each gain 1d4 negative levels.

**[[Night's Caress (Spell)|Night’s Caress]]:** Touched foe take 1d6 points of damage per level plus 1d6+2 Con damage.

**[[Nightmare (Spell)|Nightmare]]:** Sends vision dealing 1d10 damage, fatigue.

**[[Nightstalker's Transformation (Spell)|Nightstalker’s Transformation]] <sup>M</sup>:** Gain +4 Dex, +3 luck bonus to AC, +5 luck bonus on Ref saves, +3d6 sneak attack, and evasion.

## O
**O**

**[[Oath of Blood (Spell)|Oath of Blood]]:** Extends a geas beyond death that compels the target to animate and continue her quest as undead.

**[[Orb of Dancing Death (Spell)|Orb of Dancing Death]]:** Orb you control bestows 1 negative level each round.

**[[Overland Flight (Spell)|Overland Flight]]:** You fly at a speed of 40 ft. and can hustle over long distances.

## P
**P**

**[[Pain Strike, Mass (Spell)|Pain Strike, Mass]]****:** As *[[Pain Strike (Spell)|pain strike]]*, but affects multiple creatures.

**[[Passwall (Spell)|Passwall]]****:** Creates passage through wood or stone wall.

**[[Permanency (Spell)|Permanency]]** **<sup>M</sup>:** Makes certain spells permanent.

**[[Persistent Image (Spell)|Persistent Image]]****:** As *[[Major Image (Spell)|major image]]*, but with no concentration required.

**[[Phantasmal Thief (Spell)|Phantasmal Thief]]****:** Creates an unseen force that steals from others.

**[[Phantasmal Wasting (Spell)|Phantasmal Wasting]]****:** Touch causes foe to believe he has aged and become weaker.

**[[Phantasmal Web (Spell)|Phantasmal Web]]****:** Catches subjects in illusory web.

**[[Phantom Foil (Spell)|Phantom Foil]]****:** Creates phantasms that negate sneak attacks.

**[[Phantom Object, Major (Spell)|Phantom Object, Major]] <sup>R</sup>:** As *[[Major Creation (Spell)|major creation]]* except the object created is a semi-real illusion.

**[[Planar Adaptation (Spell)|Planar Adaptation]]:** Resist harmful effects of other plane.

**[[Planar Binding, Lesser (Spell)|Planar Binding, Lesser]]**: Traps extraplanar creature of 6 HD or less until it performs a task.

**[[Planar Tolerance (Spell)|Planar Tolerance]]****:** Provides long-term protection against overtly damaging planar traits.

**[[Plant Shape I (Spell)|Plant Shape I]]****:** Turns you into a Small or Medium plant.

**[[Polymorph (Spell)|Polymorph]]****:** Gives one willing subject a new form.

**[[Possible Weapon, Greater (Spell)|Possible Weapon, Greater]]****:** As *possible weapon,* but duration is longer.

**[[Power Craft, Greater (Spell)|Power Craft, Greater]]****:** As *[[Power Craft (Spell)|power craft]]**,* but lasts longer and affects a larger vehicle.

**[[Power Word Disable (Spell)|Power Word Disable]]****:** Reduces living creature with 50 hp or less to 0 hp.

**[[Precipitate Breach (Spell)|Precipitate Breach]]****:** You can force planar boundaries to rip, creating a planar breach.

**[[Prismatic Ray (Spell)|Prismatic Ray]]****:** Light ray deals damage or imparts a condition depending on its color.

**[[Prolonged Lapse of Reason]]****:** Target may only take partial actions for the duration.

**[[Prying Eyes (Spell)|Prying Eyes]]****:** 1d4 + 1/level floating eyes scout for you.

**[[Psychic Turmoil (Spell)|Psychic Turmoil]]****:** Invisible field leeches psionic power points away.

## Q
**Q**

## R
**R**

**[[Radiance (Spell)|Radiance]]****:** Creates daylight that dazzles undead.

**[[Recharge Innate Magic, Greater (Spell)|Recharge Innate Magic, Greater]]****:** You regain one use of all 0-level through 5th-level spell-like abilities.

**[[Reciprocal Gyre (Spell)|Reciprocal Gyre]]****:** Creature or object takes 1d12 damage/level of spell affecting it (max 25d12).

**[[Red Hand of the Killer (Spell)|Red Hand of the Killer]]****:** Stain the hand of a creature’s killer red.

**[[Redefine the Tools of War (Spell)|Redefine the Tools of War]]****:** Changes weapons and armor into other items of same type.

**[[Reduce Person, Greater (Spell)|Reduce Person, Greater]]****:** Subject remains reduced for 10 minutes/level.

**[[Refusal (Spell)|Refusal]]****:** Spellcasters and creatures with spell-like abilities are prevented from entering an area.

**[[Repair Light Damage, Mass (Spell)|Repair Light Damage, Mass]]****:** Repairs 1d8 damage +1/level for many constructs.

**[[Repair Undead, Mass (Spell)|Repair Undead, Mass]]:** Heals undead of 1d8 hp + 1/level; affects 1 undead/level.

**[[Resonating Resistance (Spell)|Resonating Resistance]]****:** Foes must check spell resistance twice against caster.

**[[Resounding Thunder (Spell)|Resounding Thunder]]****:** Lasting sound deals 4d6 sonic damage per round, deafens creatures within area.

**[[Retributive Image (Spell)|Retributive Image]]****:** Created illusion deals damage to those who disbelieve it.

**[[Return and Avenge (Spell)|Return and Avenge]]****:** Summoned creature returns if destroyed and attacks slayer.

**[[Revitalize Legacy, Lesser (Spell)|Revitalize Legacy, Lesser]]:** Get extra use of chosen lesser legacy ability.

## S
**S**

**[[Sapphire Screen of Shielding (Spell)|Sapphire Screen of Shielding]]****:** Moving shield of force grants +4 bonus to AC against ranged attacks and DR 10/magic against all other attacks.

**[[Scry Trap (Spell)|Scry Trap]]****:** Scry attempts against target deal 1d6/level damage and blind scrying creature.

**[[Sealed Sending (Spell)|Sealed Sending]]****:** Create a scroll bearing a message that magically appears anywhere you have viewed at least once.

**[[Secret Chest (Spell)|Secret Chest]]** **<sup>F</sup>:** Hides expensive chest on [[Ethereal Plane]]; you retrieve it at will.

**[[Secret Workshop (Spell)|Secret Workshop]]:** As *[[Nondetection (Spell)|nondetection]]* except that any number of creatures or objects in range are affected.

**[[Seeming (Spell)|Seeming]]****:** Changes appearance of one person per two levels.

**[[Sending (Spell)|Sending]]****:** Delivers short message anywhere, instantly.

**[[Shadow Evocation (Spell)|Shadow Evocation]]****:** Mimics evocation below 5th level, but only 20% real.

**[[Shadow Guardians (Spell)|Shadow Guardians]]****:** Create group of shadowy warriors.

**[[Shadow Portal (Spell)|Shadow Portal]]****:** Opens a portal to the [[Shadow, Plane of|Plane of Shadow]].

**[[Shadow Shape (midgardpf)|Shadow Shape]]****:** Gain +4 on [[Stealth (Skill)|Stealth]] and [[Escape Artist (Skill)|Escape Artist]] checks, and concealment; 

     you can move through obstacles if you have ranks in [[Escape Artist (Skill)|Escape Artist]].

**[[Shadow Space (Spell)|Shadow Space]]****:** You create an extradimensional space within your own shadow.

**[[Shadow Tentacle (Spell)|Shadow Tentacle]]****:** As *[[Shadow Tentacle, Lesser (Spell)|lesser shadow tentacle]]*, except the tentacle is larger and stronger.

**[[Shape Metal (Spell)|Shape Metal]]****:** As *[[Stone Shape (Spell)|stone shape]]*, that you affect metal instead of stone.

**[[Shard Blessing Aura (Spell)|Shard Blessing Aura]]****:** Create an aura of shimmering blue-white; this aura functions like a *[[Globe of Invulnerability, Lesser (Spell)|lesser globe of invulnerability]]**.*

**[[Shard Storm (Spell)|Shard Storm]]****:** Blast deals 3d6 damage to creatures in area.

**[[Shield Against the Balanced (Spell)|Shield Against the Balanced]]** **<sup>F</sup>:** You emit an invisible force to repel neutral-aligned outsiders.

**[[Shield Against the Contrary (Spell)|Shield Against the Contrary]]** **<sup>F</sup>:** You emit an invisible force to repel chaotic-aligned outsiders.

**[[Shield Against the Dark (Spell)|Shield Against the Dark]]** **<sup>F</sup>:** You emit an invisible force to repel evil-aligned outsiders.

**[[Shield Against the Focused (Spell)|Shield Against the Focused]]** **<sup>F</sup>:** You emit an invisible force to repel lawful-aligned outsiders.

**[[Shrieking Blast (Spell)|Shrieking Blast]]****:** 40-ft. radius burst deafens and deals 8d6 sonic damage.

**[[Shroud of Darkness (Spell)|Shroud of Darkness]]****:** Darkness descends upon the targets; treat the light level as two steps lower than normal for the duration of this spell.

**[[Shroud of Flame (Spell)|Shroud of Flame]]****:** Subject bursts into flames, taking 2d6 fire damage/round.

**[[Siphon, Greater (Spell)|Siphon, Greater]]****:** Drain 2 or more charges from wand or staff to replace one or more cast spells.

**[[Siphon Magic (Spell)|Siphon Magic]]****:** Transfers a magical effect from touched creature to yourself.

**[[Skin of the Steel Dragon (Spell)|Skin of the Steel Dragon]]****:** Grants you spell resistance equal to 10 + your caster level.

**[[Sleep Mote (Spell)|Sleep Mote]]****:** Dust devil of magic sand puts foes to sleep.

**[[Sonic Rumble (Spell)|Sonic Rumble]]** **<sup>F</sup>:** Cone of sound deals damage.

**[[Sonic Shield (Spell)|Sonic Shield]]****:** +4 deflection bonus to AC; 1d8 sonic damage and push back creatures that hit you in melee.

**[[Soul Drain (Spell)|Soul Drain]]** **<sup>XP</sup>:** Deals 1d6 +1/level damage for 1 round per caster level; if target is slain, creates a soulshard.

**[[Soul Scour (Spell)|Soul Scour]]****:** Melee touch attack deals 2d6 points of Cha and Wis damage.

**[[Soul Shackles (Spell)|Soul Shackles]]****:** Imprisons soul of dead creature in talisman, from which caster can question it.

**[[Soul Stasis (Spell)|Soul Stasis]]****:** As *[[Soul Stasis, Lesser (Spell)|lesser soul stasis]]**,* plus the subject is immune to compulsions, curses, exhaustion, possession, or wounded effects; 

     also prevents the disabled or dying conditions and all Con and Cha drain taken during its duration.

**[[Spark of Life (Spell)|Spark of Life]]****:** Undead creature loses most immunities.

**[[Speechlink, Greater (Spell)|Speechlink, Greater]]****:** You and one other creature can talk, no matter how far apart (even across planes).

**[[Spell Absorption (Spell)|Spell Absorption]]****:** Regain a previously cast spell slot equal or lower in level to any spell you successfully counterspell.

**[[Spell Gauge, Greater (Spell)|Spell Gauge, Greater]]****:** You learn the spells and spell-like abilities known or prepared by the target.

**[[Spell Haven (Spell)|Spell Haven]]** **<sup>F</sup>:** Stores one spell in your dragonmark.

**[[Spell Matrix, Lesser (Spell)|Spell Matrix, Lesser]]** **<sup>F</sup>:** Magical matrix stores a 3rd-level or lower spell to be cast later as quickened spell.

**[[Spell Theft (Spell)|Spell Theft]]****:** Dispel spells on target and gain their benefit for yourself.

**[[Spell Vulnerability, Greater (Spell)|Spell Vulnerability, Greater]]****:** Subject's spell resistance reduced by 1/caster level (max reduction 20); 

     saving throw penalty equal to your caster level.

**[[Spider Form, Greater (Spell)|Spider Form, Greater]]****:** You take on the form and abilities of a Huge fiendish monstrous spider.

**[[Spidershape (Spell)|Spidershape]]****:** You take the form of a Large monstrous spider.

**[[Spirit Fog (Spell)|Spirit Fog]]****:** Subjects in fog get –10 to Dex and Reflex checks.

**[[Spirit Self (Spell)|Spirit Self]]****:** Your incorporeal spirit separates from your body.

**[[Spiritwall (Spell)|Spiritwall]]****:** Wall of spirit-forms causes panic, deals 1d10 damage if touched, can bestow negative levels if passed through.

**[[Stolen Breath, Greater (Spell)|Stolen Breath, Greater]]** **<sup>F</sup>:** Subject has wind knocked out of it for 1 round/level.

**[[Stone Storm (Spell)|Stone Storm]]****:** Rain of bullet-sized stones pounds down, dealing bludgeoning and slashing damage.

**[[Stop Heart (Spell)|Stop Heart]]****:** Subject drops to –8 hp immediately.

**[[Storm Touch (Spell)|Storm Touch]]****:** Touch deals 9d6 electricity damage and stuns victim for 1 round; usable a number of times equal to level.

**[[Streamers (Spell)|Streamers]]****:** Ranged touch attack deals 5d10 damage, +1 streamer/three levels (max four).

**[[Stunning Breath (Spell)|Stunning Breath]]****:** Your breath weapon also stuns creatures for 1 round.

**[[Suffocation (Spell)|Suffocation]]:** Target quickly suffocates to death.

**[[Summon Monster V (Spell)|Summon Monster V]]****:** Summons extraplanar creature to fight for you.

**[[Summon Undead V (Spell)|Summon Undead V]]** **<sup>F</sup>:** Summons undead to fight for you.

**[[Suppress Legacy (Spell)|Suppress Legacy]]****:** Target temporarily loses all legacy abilities of one item.

**[[Surefooted Stride, Mass (Spell)|Surefooted Stride, Mass]]****:** As *[[Surefooted Stride (Spell)|surefooted stride]]* but multiple subjects.

**[[Symbol of Pain (Spell)|Symbol of Pain]]** **<sup>M</sup>:** Triggered rune wracks creatures with pain.

**[[Symbol of Scrying (Spell)|Symbol of Scrying]]****:** Triggered rune activates scrying sensor.

**[[Symbol of Sleep (Spell)|Symbol of Sleep]]** **<sup>M</sup>:** Triggered rune puts nearby creatures into catatonic slumber.

**[[Symbol of Spell Loss (Spell)|Symbol of Spell Loss]]** **<sup>M</sup>:** Triggered rune absorbs spells yet to be cast.

## T
**T**

**[[Taunt Dragon (Spell)|Taunt Dragon]]****:** Dragon must attack you or take a penalty on attack rolls, skill checks, and saving throws.

**[[Telekinesis (Spell)|Telekinesis]]****:** Moves object, attacks creature, or hurls object or creature.

**[[Telepathic Bond (Spell)|Telepathic Bond]]****:** Link lets allies communicate.

**[[Teleport (Spell)|Teleport]]**: Instantly transports you as far as 100 miles per level.

**[[Temporal Aging (Spell)|Temporal Aging]]****:** Target suffers a  -4 penalty to any three attributes of your choice for the duration.

**[[Terra Cotta Lion (Spell)|Terra Cotta Lion]]** **<sup>M</sup>:** Statuette becomes a Huge animated object which fights for you.

**[[Thalassemia (Spell)|Thalassemia]]****:** Turns blood to seawater, dealing 1d6/level damage.

**[[Thief Ward, Greater (Spell)|Thief Ward, Greater]]****:** [[Sleight of Hand (Skill)|Sleight of Hand]] and [[Stealth (Skill)|Stealth]] checks are made at a –20 penalty.

**[[Touch of Adamantine (Spell)|Touch of Adamantine]]****:** Weapon gains the properties of an adamantine weapon.

**[[Touch of Chaos (Spell)|Touch of Chaos]]** **<sup>M</sup>:** Touched target transforms into a mass of quivering flesh.

**[[Touch of Vecna (Spell)|Touch of Vecna]]****:** Deal negative energy damage and paralysis with your touch.

**[[Toxic Weapon (Spell)|Toxic Weapon]]****:** Coats weapon with poison.

**[[Trait Removal (Spell)|Trait Removal]]****:** Temporarily remove one extraordinary or supernatural ability of your choice from the target creature.

**[[Transferred Learning, Greater (Spell)|Transferred Learning, Greater]]****:** Transfer skill ranks or a single feat from one target to another for 1 day/level.

**[[Transformation of the Deeps (Spell)|Transformation of the Deeps]]:** Grant water breathing, darkvision, and pressure immunity to one creature/3 levels.

**[[Transmute Mud to Rock (Spell)|Transmute Mud to Rock]]****:** Transforms two 10-ft. cubes per level.

**[[Transmute Rock to Mud (Spell)|Transmute Rock to Mud]]****:** Transforms two 10-ft. cubes per level.

**[[Transmute Sand to Glass (Spell)|Transmute Sand to Glass]]****:** Transforms two 10-ft. cubes per level.

**[[Transmute Sand to Stone (Spell)|Transmute Sand to Stone]]****:** Transforms two 10-ft. cubes per level.

**[[Transmute Stone to Sand (Spell)|Transmute Stone to Sand]]****:** Transforms two 10-ft. cubes per level.

**[[Treasure Stitching (Spell)|Treasure Stitching]] <sup>M</sup>:** Objects on cloth become embroidered.

**[[Triple Strike (Spell)|Triple Strike]]****:** Grants each subject two extra attacks with a held melee weapon at its highest base attack bonus; 

   grants the *[[Keen (MagWpnSpcAb)|keen]]* and *[[Flaming Burst (MagWpnSpcAb)|flaming burst]]* abilities to melee weapons held.

## U
**U**

**[[Umber Tentacles (Spell)|Umber Tentacles]]:** Tentacles attack all creatures within a 20-ft. spread; dealing 2d6+4 damage with each successful hit.

**[[Umbral Hand (Spell)|Umbral Hand]]:** Medium hand blocks opponents or carries items.

**[[Undying Vigor of the Dragonlords (Spell)|Undying Vigor of the Dragonlords]] <sup>A</sup>:** Heal yourself of 5d6 or more points of damage.

**[[Unearthly Heat (Spell)|Unearthly Heat]]:** Target is subjected to unearthly heat for 1 round/level.

**[[Unfettered Heroism (Spell)|Unfettered Heroism]]:** Spend more than one action point per round plus gain one free action point per round.

**[[Unseen Crew (Spell)|Unseen Crew]]:** Invisible force performs the duties of one crew member on a vessel; caster gets 1 unseen crew/level.

**[[Unicorn Blood (Spell)|Unicorn Blood]]:** Gain immunity to poison, compulsion, charm effects; bestow temporary hp once.

## V
**V**

**[[Vampiric Shadow Shield (Spell)|Vampiric Shadow Shield]]****:** As *[[Fire Shield (Spell)|fire shield]]*, except attackers take negative energy damage and attacks heal you.

**[[Vanishing Weapon (Spell)|Vanishing Weapon]]****:** Weapons’ touch dispels summoned or quasi-real creatures.

**[[Vengeful Stinger (Spell)|Vengeful Stinger]]****:** Grow a stinging tail to use for attacks next to you.

**[[Village Veil (Spell)|Village Veil]]****:** Conceal an area with an illusion so that it appears to have suffered some great catastrophe 

     or calamity that renders it utterly worthless.

**[[Viscid Glob (Spell)|Viscid Glob]]****:** Ranged touch attack hurls 5-ft.-diameter glob of glue at subject.

**[[Vitriolic Sphere (Spell)|Vitriolic Sphere]]:** Potent acid deals 6d6 acid damage plus possible damage in following 2 rounds.

**[[S3.1T-Z - Universal Monster Rules T-Z (Bestiary 6 Apx)|Vulnerability]]:** Reduces an opponent’s damage reduction.

## W
**W**

**[[Wake Trailing (Spell)|Wake Trailing]]****:** You can track a ship across the sea.

**[[Wall of Dispel Magic (Spell)|Wall of Dispel Magic]]****:** Creatures passing through a transparent wall become subjects of targeted *[[Dispel Magic (Spell)|dispel magic]]*.

**[[Wall of Force (Spell)|Wall of Force]]**: Wall is immune to damage.

**[[Wall of Limbs (Spell)|Wall of Limbs]]****:** Whirling limbs deal 5d6 damage and grab creatures passing through.

**[[Wall of Magma (Spell)|Wall of Magma]]****:** Deals 2d6 fire damage out to 10 ft. and 1d6 out to 20 ft. Passing through wall deals 5d6 damage +1/level.

**[[Wall of Ooze (Spell)|Wall of Ooze]]****:** Creates barrier that has 50 hp/4 levels that paralyzes and deals 2d6 damage to all that touch it.

**[[Wall of Stone (Spell)|Wall of Stone]]****:** Creates a stone wall that can be shaped.

**[[Ward the Faithful (Spell)|Ward the Faithful]]:** Creatures of same faith gain bonuses to AC and on saving throws.

**[[Watchware (Spell)|Watchware]]****:** Mystical connection warns you if target object has been damaged, moved or touched.

**[[Waves of Fatigue (Spell)|Waves of Fatigue]]****:** Several targets become [[Fatigued|fatigued]].

**[[Whip of Centipedes (Spell)|Whip of Centipedes]]:** Create a whip made of poisonous centipedes.

**[[Whirling Weapon, Greater (Spell)|Whirling Weapon, Greater]]****:** Hurled slashing weapon magically attacks all foes in 120-ft. line.

**[[Wind Blades (Spell)|Wind Blades]]****:** You harden the air around the target into jagged invisible blades that deal damage based on how fast the target moves.

**[[Wingbind, Deadly (Spell)|Wingbind, Deadly]]****:** As *[[Hold Monster (Spell)|hold monster]]* but only paralyses the wings of a flying creature, causing it to fall. 

     The creature suffers a –2 circumstance penalty to its Will saving throw.

**[[Wrack (Spell)|Wrack]]****:** Blinds subject and renders it [[Helpless|helpless]] for 1 round/level, then –2 on attacks, saves, and checks for 3d10 minutes.

**[[Wraithstrike, Greater (Spell)|Wraithstrike, Greater]]****:** Your melee attacks strike as touch attacks for 1 round/level.

**[[Wreath of Blades (Spell)|Wreath of Blades]]** **<sup>F</sup>:** Four mithral daggers speed around you, attacking nearby creatures 

     and protecting your spellcasting from attacks of opportunity.

## X
**X**

**[[Xorn Movement (Spell)|Xorn Movement]]:** Touched creature swims through earth like a [[Xorn (Bestiary PF1)|xorn]].

**[[X-Ray Vision]]:** You see through matter.

## Y
**Y**

## Z
**Z**

**[[Zone of Peace (Spell)|Zone of Peace]]****:** As *[[Peacebond (Spell)|peacebond]]*, but affects all weapons in area. 

**[[Zone of Respite (Spell)|Zone of Respite]]:** Prevents teleportation and similar effects from functioning in the area.

**[[Zone of Revelation (Spell)|Zone of Revelation]]:** Makes invisible and ethereal creatures visible.


---
## Tags
#Rule-System/Pathfinder #Rule/Spell-List

