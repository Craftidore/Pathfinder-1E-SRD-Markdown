---
ImportedOn: "Thursday, February 16, 2023 6:10:23 PM"
Aliases:
  - "Elemental Corpus"
Category: "Spells"
Suffix: "Spell"
Tags:
  - "Category/Spells"
parent:
  - "E"
up:
  - "E"
prev:
  - "Electric Loop (Spell)"
next:
  - "Embrace the Wild (Spell)"
RWtopicId: "Topic_12737"
---
# Elemental Corpus (Spell)
## Overview
[[Transmutation Spells|Transmutation]] \[see text]

**Level:** [[7th Level Sorcerer Spells|Sorcerer 7]], [[7th Level Wizard Spells|Wizard 7]]

**BoXM:** [[BoXM 14th-Level Wizard Spells List|Wizard 14]]

**Casting Time:** 1 standard action

**Components:** V, S, M

**Range:** Personal

**Target:** You

**Duration:** 1 hour/level

**Saving Throw:** None

**Spell Resistance:** No

**Source:** Spell Compendium (WotC), pg. 78

*Drawing upon the power of the elemental planes, your transform your body in a flash of light.*

You can transform your body into a particular type of elemental substance. You and your possessions appear to be made of that element, though in the same general shape and size as your normal appearance.

You gain the following abilities when you cast *elemental corpus:*

- You have the benefits of the *[[Attune Form (Spell)|attune form]]* spell with respect to the appropriate element. If you choose a water body, for example, you can breathe normally on water-dominant planes.
- You are immune to poison, sleep, paralysis, and stunning, and are not subject to extra damage from critical hits or flanking. You gain darkvision out to 60 feet.
- Your creature type remains unchanged, so you are unaffected by spells that target elementals, but you gain the air, earth, fire, or water subtype depending on the elemental substance you chose.

In addition, you gain the following exceptional abilities according to the element chosen:

*Air*: Fly at your normal speed (perfect maneuverability), air mastery (airborne creatures take a –1 penalty on attack rolls and damage rolls against you).

*Earth:* Earth mastery (you gain a +1 bonus on attack rolls and damage rolls if both you and your foe touch the ground), push (you can start a bull rush maneuver without provoking attacks of opportunity), +3 natural armor bonus to AC.

*Fire:* Fire immunity, burn (those you hit in melee and those who attack you with natural weapons must make a Reflex save or catch fire, with a save DC equal to that of a fire elemental of the same size).

*Water:* Swim at your normal speed, water mastery (you gain a +1 bonus on attack rolls and damage rolls if both you and your opponent touch water), drench (you can use your elemental form to put out nonmagical open flames and dispel magical fire you touch as if casting *[[Dispel Magic (Spell)|dispel magic]]* at your caster level).

The *elemental corpus* spell has the descriptor of the element you choose. So, if you choose a body of fire, *elemental corpus* is a fire spell.

*Material Component:* A bit of the element in question from a plane other than the one where the spell is being cast.

**GM Note**

*House Rule:* Name changed from *elemental body* to* elemental corpus* to avoid duplication.


---
## Tags
#Sources/Spell-Compendium #Rule-System/Pathfinder #Saving-Throws/None #Rule/NonOGL #Rule/Spell #Rule/3PP #Publisher/WotC #Spell-Rarity/Rare #Spell-Resistance/No #Spell-Range/Personal #Spell-Level/Sorcerer-Wizard-7 #Spell-School/Transmutation

