---
ImportedOn: "Thursday, February 16, 2023 6:10:23 PM"
Aliases:
  - "Ghoul Glyph"
Category: "Spells"
Suffix: "Spell"
Tags:
  - "Category/Spells"
parent:
  - "G"
up:
  - "G"
prev:
  - "Ghoul Gesture (Spell)"
next:
  - "Ghoul Light (Spell)"
RWtopicId: "Topic_12876"
---
# Ghoul Glyph (Spell)
## Overview
[[Necromancy Spells|Necromancy]]

Level: [[Hunger (Domain)|Hunger 2]], [[2nd Level Sorcerer Spells|Sorcerer 2]], [[2nd Level Wizard Spells|Wizard 2]]

**BoXM:** [[BoXM 3rd-Level Cleric Spells List|Cleric 3]], [[BoXM 3rd-Level Wizard Spells List|Wizard 3]]

**Casting Time:** 1 minute

**Components:** V, S, M

**Range:** Touch

**Target:** Object touched

**Duration:** Permanent until discharged

**Saving Throw:** Fortitude partial

**Spell Resistance:** Yes

**Source:** Spell Compendium (WotC), pg. 105

*The faint, green glow of the glyph flares to life. A sickly green light fills the room, illuminating the paralyzed forms of its victims, and bringing with it the stench of death.*

You inscribe a glyph, approximately 1 foot across, that paralyzes any living creature of Large or smaller size that comes within 5 feet of the glyph. You can scribe the glyph to be visible as faintly glowing lines, or invisible. You can inscribe a *ghoul glyph* on a portable object, but if the object is moved more than 5 feet, the glyph fades.

Conditions for triggering a *ghoul glyph* are stringent. It takes effect on any creature except yourself that moves within 5 feet. It affects invisible creatures normally but is not triggered by those that travel past it ethereally. Only a single *ghoul glyph* can be inscribed in a 5-foot square.

*Ghoul glyphs* cannot be affected or bypassed by such means as physical or magical probing, though they can be dispelled. *[[Mislead (Spell)|Mislead]]* and *[[Nondetection (Spell)|nondetection]]* can fool a *ghoul glyph*.

*[[Read Magic (Spell)|Read magic]]* allows identification of a *ghoul glyph* with a successful DC 13 [[Spellcraft (Skill)|Spellcraft]] check, if the glyph is noticed before it is activated. A rogue can use the [[Perception (Skill)|Perception]] (3.5 Search) skill to find a *ghoul glyph* and [[Disable Device (Skill)|Disable Device]] to thwart it. The DC in each case is 27.

When a glyph is activated, the target is paralyzed for 1d6+2 rounds. Additionally, if the target fails a Fortitude save, the paralyzed subject exudes a carrion stench that causes distress in all creatures within a 10-foot radius. Those in the radius, including the target, must make a Fortitude save or take a –2 penalty on attack rolls, weapon damage rolls, saving throws, skill checks, and ability checks until the paralysis effect wears off.

*Material Component:* You trace the glyph with earth from a ghoul’s lair.


---
## Tags
#Saving-Throws/Fortitude-Partial #Spell-School/Necromancy #Spell-Level/Sorcerer-Wizard-2 #Spell-Range/Touch #Domain/Hunger #Spell-Resistance/Yes #Sources/Spell-Compendium #Rule-System/Pathfinder #Rule/NonOGL #Rule/Spell #Rule/3PP #Publisher/WotC #Spell-Rarity/Rare

