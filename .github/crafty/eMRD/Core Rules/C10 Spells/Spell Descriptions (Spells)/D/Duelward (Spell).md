---
ImportedOn: "Thursday, February 16, 2023 6:10:23 PM"
Aliases:
  - "Duelward"
Category: "Spells"
Suffix: "Spell"
Tags:
  - "Category/Spells"
parent:
  - "D"
up:
  - "D"
prev:
  - "Disguise Ship (Spell)"
next:
  - "Divine Retribution (Spell)"
RWtopicId: "Topic_7098"
---
# Duelward (Spell)
## Overview
[[Abjuration Spells|Abjuration]]

**Level:** [[5th Level Sorcerer Spells|Sorcerer 5]], [[5th Level Wizard Spells|Wizard 5]]

**BoXM:** [[BoXM 9th-Level Wizard Spells List|Wizard 9]]

**Casting Time:** 1 standard action

**Components:** V, S, M (a miniature silk glove)

**Range:** Personal

**Target:** You

**Duration:** 1 round/level or until discharged (D)

**Saving Throw:** None

**Spell Resistance:** No

**Source:** Spell Compendium (WotC), pg. 75

*The air around you crackles with magical energy as you finish uttering the last syllables of the spell’s formula. You immediately feel as if your link to the arcane somehow has increased, filling you with confidence and a sense of security.*

While a *duelward* spell is in effect, counterspelling is an immediate action for you, allowing you to counterspell even when it is not your turn without having previously readied an action. You also gain a +4 competence bonus on [[Spellcraft (Skill)|Spellcraft]] checks made to identify spells being cast.

The first time you successfully counterspell while the spell is in effect (whether you counterspell as an immediate action or not), *duelward* is discharged.


---
## Tags
#Sources/Spell-Compendium #Rule/Spell #Publisher/WotC #Rule/fm3 #Rule/NonOGL #Spell-Rarity/Rare #Spell-Resistance/No #Rule-System/Pathfinder #Spell-School/Abjuration #Saving-Throws/None #Spell-Range/Personal #Rule/3PP

