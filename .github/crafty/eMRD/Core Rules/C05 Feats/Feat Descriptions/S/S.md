---
ImportedOn: "Thursday, February 16, 2023 6:10:23 PM"
Category: "Feats"
Tags:
  - "Category/Feats"
parent:
  - "Feat Descriptions"
up:
  - "Feat Descriptions"
prev:
  - "R"
next:
  - "T"
down:
  - "Spell Perfection (Feat)"
  - "Secret of Magical Discipline (Feat)"
  - "Staff-Like Wand (Feat)"
  - "Skewering Spear (Feat)"
  - "Spear Wind (Feat)"
  - "Spear Wind Mastery (Feat)"
  - "Spell Thematics (Feat)"
  - "Spontaneous Casting (Feat)"
  - "Shatter Defenses (Feat)"
  - "Suspend Spell (Feat)"
  - "Sanctum Spell (Feat)"
  - "Spell Exchange (Feat)"
  - "Sacred Geometry (Feat)"
  - "Spontaneous Healer (Feat)"
  - "Scribe Scroll (Feat)"
  - "Spell Mastery (Feat)"
  - "Still Spell (Feat)"
  - "Scorpion Style (Feat)"
  - "Selective Channeling (Feat)"
  - "Self-Sufficient (Feat)"
  - "Shield Focus (Feat)"
  - "Shield Master (Feat)"
  - "Shield Proficiency (Feat)"
  - "Shield Slam (Feat)"
  - "Shot on the Run (Feat)"
  - "Sickening Critical (Feat)"
  - "Simple Weapon Proficiency (Feat)"
  - "Skill Focus (Feat)"
  - "Snatch Arrows (Feat)"
  - "Spirited Charge (Feat)"
  - "Staggering Critical (Feat)"
  - "Stand Still (Feat)"
  - "Stealthy (Feat)"
  - "Step Up (Feat)"
  - "Strike Back (Feat)"
  - "Stunning Critical (Feat)"
  - "Stunning Fist (Feat)"
  - "Spellbreaker (Feat)"
  - "Spring Attack (Feat)"
  - "Spell Mantle (Feat)"
  - "Spell Mantle, Greater (Feat)"
  - "Spear Dancer (Feat)"
  - "Spear Dancing Style (Feat)"
  - "Spear Dancing Spiral (Feat)"
  - "Spear Dancing Reach (Feat)"
  - "Storm Bolt (Feat)"
  - "Stabbing Shot (Feat)"
  - "Snap Shot (Feat)"
  - "Sharp-Shooting (Feat)"
  - "Second Bond (Feat)"
  - "Signature Spell (Feat)"
  - "Swift Crafting (Feat)"
  - "Surehand (Feat)"
  - "Strike of the Viper (Feat)"
  - "Steadfast Loyalty (Feat)"
  - "Suspicious Eye (Feat)"
  - "Student of Nature (Feat)"
  - "Synchronized Shot (Feat)"
  - "Steadfast Mind (Feat)"
  - "Spell Focus (Feat)"
  - "Spell Penetration (Feat)"
  - "Silent Spell (Feat)"
  - "Spell Finesse (Feat)"
  - "Sanctify Relic (Feat)"
  - "Seething Hatred (Feat)"
  - "Seize Advantage (Feat)"
  - "Share Healing (Feat)"
  - "Silent Kill (Feat)"
  - "Skald's Vigor (Feat)"
  - "Skilled Rager (Feat)"
  - "Slashing Grace (Feat)"
  - "Slayer's Feint (Feat)"
  - "Slow Faller (Feat)"
  - "Spirit Talker (Feat)"
  - "Spirit's Gift (Feat)"
  - "Spiritual Guardian (Feat)"
  - "Spontaneous Nature's Ally (Feat)"
  - "Staggering Fist (Feat)"
  - "Stalker's Focus (Feat)"
  - "Steadfast Personality (Feat)"
  - "Stouthearted (Feat)"
  - "Studied Combatant (Feat)"
  - "Surprise Maneuver (Feat)"
  - "Shield Specialization (Feat)"
RWtopicId: "Topic_16318"
---
# S
## Overview
| **Feats** | **Prerequisites** | **Benefits** |
|---|---|---|
| Sabotage Magic Item | [[Magical Aptitude (Feat)\|Magical Aptitude]], [[Disable Device (Skill)\|Disable Device]] 5 ranks, [[Use Magic Device (Skill)\|Use Magic Device]] 5 ranks | Sabotage an item to cause a mishap |
| Sabotage Specialist | [[Deft Hands (Feat)\|Deft Hands]], [[Disable Device (Skill)\|Disable Device]] 5 ranks | Items you sabotage fail at a more precise time |
| Sabotaging Sunder\* | Str 13, [[Improved Sunder (Feat)\|Improved Sunder]], [[Power Attack (Feat)\|Power Attack]], [[Disable Device (Skill)\|Disable Device]] 7 ranks | Break objects through sabotage rather than force |
| [[Sacred Geometry (Feat)\|Sacred Geometry]] | Int 13, [[Knowledge (Skill)\|Knowledge]] (engineering) 2 ranks | Add metamagic effects to your spells without using a higher-level spell slot |
| Sacrificial Adept | Knowledge (arcana) 4 ranks, Knowledge (planes) 4 ranks, Knowledge (religion) 4 ranks, Spellcraft 4 ranks, ability to cast 3rd-level spells, evil alignment | Kill sentient creatures to enhance your spells |
| Sacrificial Ritualist | Mutilating Ritualist | Kill sentient creatures to enhance your ritual |
| Sanctify Relic | Any other item creation feat, caster level 7th | Make magic items with a divine connection |
| Sanctum Spell | Any metamagic feat | Spell’s effective level is increased while in special location, decreased outside special location |
| Sap Adept\* | Sneak attack +1d6 | Gain bonus damage on nonlethal attacks |
| Sap Master\* | Sap Adept, sneak attack +3d6 | Deal twice your sneak attack damage when dealing nonlethal damage |
| Saving Shield\* | Shield Proficiency | Grant shield bonus to an adjacent ally |
| School Strike\* | Improved Unarmed Strike, wizard school class feature | May deliver magic school effect as swift action after melee attack |
| Scion of Sorrow | Neutral evil alignment | Gain +1 luck bonus on an attack roll, skill check, ability check, level check, or saving throw 1/day |
| Scorpion Style\* | Improved Unarmed Strike | Reduce target’s speed to 5 ft. |
| Sea Legs | Profession (sailor) 5 ranks | Gain +2 on Acrobatics, Climb, and Swim checks |
| Second Bond | Bond class feature, character level 5 | Gain a second class bond |
| Second Chance\* | Combat Expertise, base attack bonus +6 | Exchange later attacks to reroll missed first attack |
| Seething Hatred\* | Studied target class feature | Select a favored enemy and gain greater benefits when such a creature is your studied target |
| Seize Advantage\* | Opportune parry and riposte deed | Add foe’s Power Attack damage to your riposte |
| Selective Channeling | Cha 13, channel energy class feature | Choose whom to affect with channel energy |
| Self-Sufficient | — | +2 bonus on Heal and Survival checks |
| Sense Assumptions | Sense Motive 3 ranks | Discern which lies would be unbelievable |
| Sense Relationships | Street Smarts | Discern relationship between two people and use it against them |
| Sentinel Stance | Any Mark of Sentinel | Reroll a Sense Motive check 1/day; allies adjacent to you gain bonus to AC and on Reflex saves |
| Serpent Strike | Simple Weapon Proficiency (longspear), Weapon Focus (longspear), flurry of blows | Treat longspear as monk weapon |
| Shadow Strike\* | Base attack bonus +1 | Deal precision damage against targets with concealment |
| Shadow Veil | Ability to cast 2nd-level spells | Obscure a target's sight, +1 CL on darkness spells |
| Shadows of Fear\* | Hidden strike +2d8 or sneak attack +2d6 | Flank a scared creature once per round |
| Shapeshifter Foil | Knowledge (arcana) 5 ranks or Knowledge (nature) 5 ranks, ability to use polymorph effects | When you deal damage, target’s ability to use any polymorph effects is hindered |
| Shapeshifting Hunter | Favored enemy class feature, wild shape class feature | Add your druid and ranger levels for favored enemy |
| Share Healing | Ability to acquire an animal companion, eidolon, familiar, or special mount | Share healing with your companion creature |
| Shared Insight | Wis 13, half-elf | Grant friendly creatures a +2 bonus on Perception checks |
| Shared Judgment | Second judgment class feature | Extend a judgment to an adjacent ally instead of enacting a second judgment |
| Shared Soul | Shared consciousness class feature | Phantom protects against death effects, energy drain, and possession |
| Sharp Senses | Keen senses racial trait | +4 racial bonus on Perception checks |
| Sharp-Shooting\* | Point Blank Shot, Precise Shot, base attack bonus +3 | Halve opponent’s cover bonus |
| Shatter Control | Channel positive energy 4d6 | Use channel to cut the control of undead creatures |
| Shatter Defenses\* | Dazzling Display, base attack bonus +6 | Hindered foes are flat-footed |
| Shatter Mental Mask | Psychic Sensitivity or ability to cast psychic spells or use psychic spell-like abilities, character level 2nd | Offensive manifestations with Will saves reveal opponent’s true form |
| Shield Focus\* | Shield Proficiency, base attack bonus +1 | Gain a +1 bonus to your AC when using a shield |
| Shield Master\* | Shield Slam, base attack bonus +11 | No two-weapon penalties when attacking with a shield |
| Shield of Siberys | Any true dragonmark | Sense aberrant dragonmarked creatures within 60 ft.; gain spell resistance against aberrant mark powers |
| Shield of Swings\* | St bvnr 13, Power Attack, base attack bonus +1 | Reduce damage with a two handed weapon to gain a +4 shield bonus |
| Shield Proficiency\* | — | No penalties on attack rolls when using a shield |
| Shield Slam\* | Improved Shield Bash, Two-Weapon Fighting, base attack bonus +6 | Free bull rush with a bash attack |
| Shield Sling\* | Proficiency with shields, Improved Shield Bash, Shield Specialization, base attack bonus +9 | Use shield as ranged weapon |
| Shield Specialization\* | Shield Focus, 4th-level fighter | +2 bonus to AC against criticals with one type of shield |
| Shield Ward\* | Proficiency with shields, Shield Specialization | Gain shield bonus to touch AC and against bull rush, disarm, grapple, overrun, and trip |
| Shipwright | Craft (ships) 3 ranks, Knowledge (engineering) 3 ranks | Gain +2 on Craft (ships) and reduce costs for building or repairing any sort of boat, ship, or watercraft |
| Short Haft\* | Proficiency with selected weapon, Weapon Focus with selected weapon, base attack bonus +3 | Lose reach benefits, attack adjacent foes |
| Shot on the Run\* | Dex 13, Mobility, Point-Blank Shot, base attack bonus | +4 Make ranged attack at any point during movement |
| Sickening Critical\* | Critical Focus, base attack bonus +11 | Whenever you score a critical hit, the target is sickened |
| Sickening Grasp | Ability to cast 3rd-level spells | Sicken a target with a touch, +1 CL on necromancy spells |
| Sidestep\* | Dex 13, Dodge, Mobility | Make 5-foot step immediately after an opponent misses |
| Siege Commander\* | Craft (siege weapon) 5 ranks, Knowledge (engineering) 5 ranks or Profession (Siege Engineer) 1 rank | Grant weapon crew +4 on assemble or move checks |
| Siege Gunner\* | Siege Weapon Engineer | Take no size penalties for aiming a direct fire siege weapon |
| Siege Weapon Engineer\* | Knowledge (engineering) 5 ranks or Profession (siege engineer) 5 ranks, proficiency with 1 siege weapon | You are considered proficient with all siege weapons |
| Signature Spell | Spell Mastery or a domain | Sacrifice a prepared spell to cast your signature spell |
| Silent Kill\* | Stealth 12 ranks, assassinate advanced slayer talent or ninja master trick | Kill during the surprise round and slip back into shadows |
| Silver Smite | Follower of Silver Flame, smite evil | Add 1d6 points of sacred damage when smiting evil |
| Simple Weapon Proficiency\* | — | No penalty on attacks made with simple weapons |
| Skald’s Vigor | Raging song class feature | Gain fast healing while raging and performing or performing raging song |
| Skewering Spear \* | Lunge, Weapon Focus (longspear or spear), Weapon Specialization (longspear or spear) | Increase the critical multiplier of your spear by one step whenever you use the Lunge feat |
| Skill Focus | — | +3 bonus on one skill (+6 at 10 ranks) |
| Skilled Driver | — | Gain +4 bonus on driving checks with made with one vehicle |
| Skilled Rager | Rage class feature | Pick a skill and use it while raging |
| Slashing Grace\* | Dex 13, Weapon Finesse, Weapon Focus with selected weapon | Treat a one-handed slashing weapon as a piercing melee weapon |
| Slayer’s Feint\* | Dex 15, Acrobatic or slayer level 1st, Combat Expertise, Acrobatics 1 rank | Use Acrobatics instead of Bluff to feint |
| Slayer’s Knack | Favored enemy class feature, base attack bonus +6 | Double the threat range of all weapons against one favored enemy |
| Sliding Dash\* | Dex 15, Acrobatics 10 ranks or acrobatic charge class feature, Bluff 3 ranks | Charge through a foe for a flank attack |
| Sling Flail Weapon | Focus (sling), base attack bonus +1 | Make melee attacks with a sling |
| Slow Faller | Acrobatics 5 ranks or slow fall class feature | Gain or improve the slow fall ability |
| Smash\* | Power Attack, half-orc | Ignore 5 points of hardness when breaking an object |
| Smell Fear | Keen Scent, half-orc or orc | +4 on Perception checks to detect frightened creatures |
| Snap Shot\* | Rapid Shot, Weapon Focus, base attack bonus +6 | Threaten squares within 5 feet of you when wielding a ranged weapon |
| Snatch Arrows\* | Dex 15, Deflect Arrows | Catch one ranged attack per round |
| Sneaking Precision | Sneak attack class feature, Critical Focus, any critical feat, base attack bonus +9 | On the second sneak attack in a round, you may apply a critical feat |
| Sociable | Cha 13, half-elf | Grant friendly creatures a +2 bonus on Diplomacy checks |
| Social Bravery\* | Cha 13, bravery class feature | Apply bravery against various social attacks |
| Somatic Weaponry | Concentration 5 ranks, Spellcraft 5 ranks | Cast spells with somatic components while hands are full |
| Song of the Heart | Bardic music, inspire competence, Perform 6 ranks | Increase bardic music effects |
| Soothe the Beast | Bardic music, Perform 6 ranks | Bardic music improves the attitudes of animals |
| Sorcerous Strike | Sorcerer bloodline class feature, Improved Unarmed Strike | Apply a bloodline ability as part of a melee attack |
| Spear Dancer | Weapon Focus (any two-handed reach weapon), Perform (dance) 4 ranks | When you hit a creature with a two-handed reach weapon they are dazzled for 1 round |
| Spear Dancing Reach \* | Dex 17, Spear Dancing Spiral, Spear Dancing Style, Two-Weapon Fighting, Weapon Finesse, Weapon Focus (with the chosen weapon) | Strike from afar with double weapons |
| Spear Dancing Spiral \* | Dex 15, Spear Dancing Style, Two-Weapon Fighting, Weapon Finesse, Weapon Focus (with the chosen weapon) | Wield spears with poise and grace |
| Spear Dancing Style \* | Dex 13, Two-Weapon Fighting, Weapon Focus (with the chosen weapon) | Use polearms and spears as double weapons |
| Spear Wind \* | Elf, Dex 13, Weapon Focus (any spear), base attack bonus +4 | Make one extra attack per round; all attacks take a -3 penalty |
| Spear Wind Mastery \* | Elf, Dex 13, Spear Wind, Weapon Focus (any spear), base attack bonus +8 | +2 dodge bonus to AC when using Spear Wind feat with the full attack action |
| Spell Bane | Bane class feature | Your spells’ DCs increase by 2 when bane is in effect |
| Spell Exchange | Wizard level 3rd+ | Swap a spell in place of one of equal or higher level |
| Spell Finesse | — | Choose one of the following ability scores: Intelligence, Wisdom or Charisma; all you spellcasting calculations are now based on this ability |
| Spell Focus | — | +1 bonus on save DCs for one school |
| Spell Focus (Good) | Any good alignment | +2 bonus to save DCs of spells with good descriptor |
| Spell Mantle | Craft Contingent Spell, Knowledge (arcana) 5 ranks, Knowledge (history) 2 ranks | You can create a mantle from the contingent spells you care |
| Spell Mastery | 1st-level Wizard | Prepare some spells without a spellbook |
| Spell Penetration | — | +2 bonus on level checks to beat spell resistance |
| Spell Perfection | Spellcraft 15 ranks, 3 metamagic feats | Apply any metamagic feat to one spell without penalty, up to 9th level |
| Spellbreaker\* | Disruptive, 10th-level fighter | Enemies provoke attacks if their spells fail |
| Spider Step | Acrobatics 6 ranks, Climb 6 ranks, 6th-level monk | Walk on walls or ceilings for half your slow fall distance |
| Spinning Throw\* | Improved Bull Rush, Improved Trip, Improved Unarmed Strike, Ki Throw | Spend a swift action to bull rush opponent, move it, then knock it prone |
| Spirit Focus | Spirit bonus class feature | Increase spirit bonus from one legend by 1 |
| Spirit Sense | Psychic Sensitivity or ability to cast psychic spells | +5 on checks to notice haunts, 75% chance to affect incorporeal creatures |
| Spirit Speaker | Cha 13, Absorb Spirit, Diplomacy 5 ranks | Talk to spirits you’ve absorbed |
| Spirit Talker | Hex class feature; shaman level 6th or witch level 6th | Commune with a spirit to gain temporary use of one of its hexes |
| Spirit’s Gift | Animal companion or familiar class feature | Commune with a spirit to grant your animal companion or familiar a shaman spirit augmentation |
| Spirited Charge\* | Ride-By Attack | Double damage on a mounted charge |
| Spiritual Balance | Ki pool class feature, still mind class feature | Still mind applies against possession, spend ki to double the bonus |
| Spiritual Guardian | Ability to cast spiritual weapon or spiritual ally, spirit magic class feature | Increase the attack power of your spirit spells |
| Spiritualist’s Call | Phantom class feature | Phantom gets a bonus when summoned |
| Splintering Weapon | Weapon Proficiency, base attack bonus +1 | Break a weapon to deal bleed damage |
| Spontaneous Casting | Caster 5th | Swap one prepared spell for another spell on spell list |
| Spontaneous Healer | Knowledge (religion) 4 ranks, nonevil | Swap out your spells for *cure* spells on your list |
| Spontaneous Nature’s Ally | Knowledge (nature) 5 ranks, ability to spontaneously cast cure or inflict spells, Animal or Plant domain | Lose a spell slot to spontaneous cast a *summon nature’s ally* spell of the same level |
| Spring Attack\* | Dodge, Mobility, base attack bonus +4 | Move before and after melee attack |
| Stabbing Shot\* | Rapid Shot, elf | Use arrow as a melee attack to push enemies back |
| Staff-Like Wand | Craft Staff, wizard 11+ | Use your Int modifier, caster level and relevant feats to set the DC for saves when using a wand |
| Stage Combatant\* | Weapon Focus, base attack bonus +5 | Take no penalty to deal nonlethal damage |
| Staggering Critical\* | Critical Focus, base attack bonus +13 | Whenever you score a critical hit, the target is staggered |
| Staggering Fist\* | Dex 13, Wis 13, Improved Unarmed Strike, base attack bonus | +2 Stagger foes with unarmed strikes |
| Stalker’s Focus | Knowledge (nature) 3 ranks, Survival 3 ranks, no ranks in a class that has the animal focus class feature | Gain an animal focus for you or your animal companion |
| Stalwart | Diehard, base attack bonus +4 | Forgo dodge AC bonus for equivalent DR |
| Stand Still\* | Combat Reflexes | Stop enemies from moving past you |
| Starry Grace\* | Dex 13, Weapon Finesse, Weapon Focus (starknife) | Use Dexterity on starknife damage rolls |
| Startling Getaway\* | Startling appearance class feature | Move away after a startling appearance |
| Steadfast Loyalty | — | Gain +2 bonus on Will saving throws against charm, compulsion, and fear effects |
| Steadfast Mind | Cha 15, Int 15, or Wis 15 (see special) | Possibly retain a spell that would be lost when failing a Concentration check while casting defensively |
| Steadfast Personality | — | Gain an insight bonus on saves against mind-affecting effects equal to your Charisma modifier |
| Stealthy | — | +2 bonus on Escape Artist and Stealth checks |
| Steel Soul | Dwarf, hardy racial trait | +4 racial bonus on saves against spells and spell-like abilities |
| Step Up\* | Base attack bonus +1 | Take a 5-foot step as an immediate action |
| Step Up and Strike\* | Following Step, base attack bonus +6 | Follow adjacent creature and attack as an immediate action |
| Stone Sense | Perception 10 ranks, Improved Stonecunning | Gain tremorsense 10 ft. |
| Stone Singer | Cha 13, bardic performance, dwarf | Gain benefits to bardic performance underground and against earth creatures |
| Stone-Faced | Dwarf +4 bonus on | Bluff checks to lie or conceal motives |
| Strong Comeback | — | Add a +2 bonus on all rerolls |
| Storm Bolt | Ability to cast 3rd-level spells | Create line of electricity 1d6 damage/spell level, +1 CL on electricity spells |
| Storm’s Riposte | Any Mark of Storm | Spend 1 action point to attack a foe that has just attacked you, deal extra electricity or sonic damage |
| Stormrider | Any Mark of Storm | Reroll Balance check 1/day, take 10 on Balance checks when threatened or distracted; gain resistance to electricity and sonic |
| Stouthearted | Raging song class feature | Use raging song to reroll failed saves against fear effects |
| Strangler\* | Improved Grapple, sneak attack +1d6 | Deal sneak damage to grappled opponent |
| Street Smarts | — | +2 bonus on Knowledge (local) and Sense Motive checks |
| Strike Back\* | Base attack bonus +11 | Attack foes that strike you while using reach |
| Strike of the Viper\* | Base attack bonus +5, Quick Draw | First attack using Quick Draw on a flat-footed opponent triples the weapon's threat range |
| Strong Implement Link | Implement class feature | Cast spells away from an implement more easily |
| Strong Mind | Wis 11 | +3 on saves against psionics |
| Stubborn Curse | Ability to cast a spell or spell-like ability with the curse descriptor or a special ability with a curse effect | Creatures roll twice to remove your curses |
| Student of Nature | — | +1 bonus on Handle Animal, Knowledge (nature), and Survival skill checks; also get a +2 bonus on Fortitude saving throws to resist special attacks from plant creatures |
| Studied Combatant | Amateur Investigator, base attack bonus +6 | Expend inspiration to gain a limited version of studied combat |
| Stunning Assault\* | Power Attack, base attack bonus +16 | Trade melee attack bonus to stun opponents |
| Stunning Critical\* | Staggering Critical, base attack bonus +17 | Whenever you score a critical hit, the target is stunned |
| Stunning Fist\* | Dex 13, Wis 13, Improved Unarmed Strike, base attack bonus +8 | Stun opponent with an unarmed strike |
| Stunning Pin\* | Improved Grapple, Stunning Fist | Use Stunning Fist against pinned opponents |
| Subduing Strike\* | — | No penalty when dealing nonlethal damage; rogue’s sneak attack deals nonlethal damage |
| Subtle Enchantments | Deceitful, Spell Focus (enchantment) | Victims and observers might not notice your enchantments |
| Summon Elemental | Ability to cast 4th-level spells | Summon elemental, +1 CL on summoning spells |
| Summoner’s Call | Eidolon class feature | Your eidolon gets a bonus when summoned |
| Sundering Strike\* | Improved Sunder, base attack bonus +9 | Attempt a sunder attack on a successful critical hit |
| Sunlight Eyes | Ability to cast 2nd-level spells | See in darkness 10 feet/spell level, +1 CL on light spells |
| Superior Scryer | Spell Focus (divination), the ability to cast at least one spell of the scrying subschool | Improve various effects of scrying spells |
| Sure Grasp | Climb 1 rank | When climbing, roll twice and take the highest result |
| Surehand | Base attack bonus +1, Str or Dex 14+ | Use melee weapons one size category larger than you are with a –1 penalty on the attack roll |
| Surprise Maneuver | Combat Expertise; sneak attack +3d6 or studied strike +3d6 | Gain a bonus on combat maneuver checks against flanked foes and those denied their Dex bonus to AC |
| Suspicious Eye | — | +2 bonus on Sense Motive skill checks |
| Swift Aid\* | Combat Expertise, base attack bonus +6 | Attempt to aid another as a swift action |
| Swift Crafting | Craft 5 ranks or any item creation feat | Craft magical items faster |
| Swipe and Stash\* | Sleight of Hand 5 ranks | Plant objects on others |
| Sword And Pistol\* | Snap Shot, Two-Weapon Fighting, base attack bonus +6 | Ranged attacks do not provoke attacks of opportunity |
| Synchronized Shot\* | Point Blank Shot, Precise Shot | Your ranged attacks ignore cover from allies |

  \* This is a combat feat, and can be selected as a brawler, fighter, gunslinger, swashbuckler, and warpriest bonus feat.

---
## Governed Content
- [[Sacred Geometry (Feat)]]
- [[Sanctify Relic (Feat)]]
- [[Sanctum Spell (Feat)]]
- [[Scorpion Style (Feat)]]
- [[Scribe Scroll (Feat)]]
- [[Second Bond (Feat)]]
- [[Secret of Magical Discipline (Feat)]]
- [[Seething Hatred (Feat)]]
- [[Seize Advantage (Feat)]]
- [[Selective Channeling (Feat)]]
- [[Self-Sufficient (Feat)]]
- [[Share Healing (Feat)]]
- [[Sharp-Shooting (Feat)]]
- [[Shatter Defenses (Feat)]]
- [[Shield Focus (Feat)]]
- [[Shield Master (Feat)]]
- [[Shield Proficiency (Feat)]]
- [[Shield Slam (Feat)]]
- [[Shield Specialization (Feat)]]
- [[Shot on the Run (Feat)]]
- [[Sickening Critical (Feat)]]
- [[Signature Spell (Feat)]]
- [[Silent Kill (Feat)]]
- [[Silent Spell (Feat)]]
- [[Simple Weapon Proficiency (Feat)]]
- [[Skald's Vigor (Feat)]]
- [[Skewering Spear (Feat)]]
- [[Skill Focus (Feat)]]
- [[Skilled Rager (Feat)]]
- [[Slashing Grace (Feat)]]
- [[Slayer's Feint (Feat)]]
- [[Slow Faller (Feat)]]
- [[Snap Shot (Feat)]]
- [[Snatch Arrows (Feat)]]
- [[Spear Dancer (Feat)]]
- [[Spear Dancing Reach (Feat)]]
- [[Spear Dancing Spiral (Feat)]]
- [[Spear Dancing Style (Feat)]]
- [[Spear Wind (Feat)]]
- [[Spear Wind Mastery (Feat)]]
- [[Spell Exchange (Feat)]]
- [[Spell Finesse (Feat)]]
- [[Spell Focus (Feat)]]
- [[Spell Mantle (Feat)]]
- [[Spell Mantle, Greater (Feat)]]
- [[Spell Mastery (Feat)]]
- [[Spell Penetration (Feat)]]
- [[Spell Perfection (Feat)]]
- [[Spell Thematics (Feat)]]
- [[Spellbreaker (Feat)]]
- [[Spirit Talker (Feat)]]
- [[Spirited Charge (Feat)]]
- [[Spirit's Gift (Feat)]]
- [[Spiritual Guardian (Feat)]]
- [[Spontaneous Casting (Feat)]]
- [[Spontaneous Healer (Feat)]]
- [[Spontaneous Nature's Ally (Feat)]]
- [[Spring Attack (Feat)]]
- [[Stabbing Shot (Feat)]]
- [[Staff-Like Wand (Feat)]]
- [[Staggering Critical (Feat)]]
- [[Staggering Fist (Feat)]]
- [[Stalker's Focus (Feat)]]
- [[Stand Still (Feat)]]
- [[Steadfast Loyalty (Feat)]]
- [[Steadfast Mind (Feat)]]
- [[Steadfast Personality (Feat)]]
- [[Stealthy (Feat)]]
- [[Step Up (Feat)]]
- [[Still Spell (Feat)]]
- [[Storm Bolt (Feat)]]
- [[Stouthearted (Feat)]]
- [[Strike Back (Feat)]]
- [[Strike of the Viper (Feat)]]
- [[Student of Nature (Feat)]]
- [[Studied Combatant (Feat)]]
- [[Stunning Critical (Feat)]]
- [[Stunning Fist (Feat)]]
- [[Surehand (Feat)]]
- [[Surprise Maneuver (Feat)]]
- [[Suspend Spell (Feat)]]
- [[Suspicious Eye (Feat)]]
- [[Swift Crafting (Feat)]]
- [[Synchronized Shot (Feat)]]

