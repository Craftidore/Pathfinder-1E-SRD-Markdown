---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Aliases:
  - "Temporary Resurrection"
Category: "Spells"
Tags:
  - "Category/Spells"
parent:
  - "T"
up:
  - "T"
prev:
  - "Threshold Barrier (Spell)"
RWtopicId: "Topic_19897"
---
# Temporary Resurrection (Spell)
## Overview
[[Necromancy Spells|Necromancy]]

**Level:** [[7th Level Sorcerer Spells|Sorcerer 7]], [[7th Level Witch Spells|Witch 7]], [[7th Level Wizard Spells|Wizard 7]]

**BoXM:** [[BoXM 13th-Level Wizard Spells List|Wizard 13]]

**Casting Time:** 10 minutes

**Components:** V, S, M (diamond dust worth 500 gp)

**Range:** Touch

**Target:** Dead creature touched

**Duration:** 24 hours

**Saving Throw:** None

**Spell Resistance:** No

**Source:** Ultimate Magic, pg. 243

You restore temporary life to a body that has been dead for less than 48 hours. The spell lasts for 24 hours, after which the target dies again. The target gains 1 permanent negative level while under the effect of this spell; this negative level goes away when the target dies or is permanently raised from the dead (such as with *[[Raise Dead (Spell)|raise dead]]*). The target still counts as a dead corpse (but not undead) for the purpose of spells that revive dead creatures, so a cleric can cast *[[Raise Dead (Spell)|raise dead]]* or a similar spell on the target even while this spell is active. Once a creature has been revived with *temporary resurrection*, this spell cannot be used on it again until it is permanently raised from the dead.


---
## Tags
#Rule/OGL #Spell-Resistance/No #Sources/Ultimate-Magic #Rule-System/Pathfinder #Spell-School/Necromancy #Saving-Throws/None #Spell-Level/Sorcerer-Wizard-7 #Spell-Range/Touch #Spell-Level/Witch-7 #Rule/Spell #Publisher/Paizo #Rule/Core #Spell-Rarity/Uncommon #Rule/fm3

