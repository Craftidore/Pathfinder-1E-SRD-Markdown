---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Aliases:
  - "Telepathic Bond"
  - "Rary's telepathic bond"
Category: "Spells"
Tags:
  - "Category/Spells"
parent:
  - "T"
up:
  - "T"
prev:
  - "Telekinetic Sphere (Spell)"
next:
  - "Teleport, Greater (Spell)"
RWtopicId: "Topic_14573"
---
# Telepathic Bond (Spell)
## Overview
[[Divination Spells|Divination]]

**Level:** [[5th Level Sorcerer Spells|Sorcerer 5]], [[5th Level Wizard Spells|Wizard 5]]

**BoXM:** [[BoXM 9th-Level Wizard Spells List|Wizard 9]]

**Casting Time:** 1 standard action

**Components:** V, S, M (two eggshells from two different creatures)

**Range:** Close (25 ft. + 5 ft./2 levels)

**Targets:** You plus one willing creature per three levels, no two of which can be more than 30 ft. apart

**Duration:** 10 minutes/level (D)

**Saving Throw:** None

**Spell Resistance:** No

**Source:** PF Core Rulebook, pg. 358

You forge a telepathic bond among yourself and a number of willing creatures, each of which must have an Intelligence score of 3 or higher. Each creature included in the link is linked to all the others. The creatures can communicate telepathically through the bond regardless of language. No special power or influence is established as a result of the bond. Once the bond is formed, it works over any distance (although not from one plane to another).

If desired, you may leave yourself out of the telepathic bond forged. This decision must be made at the time of casting.

*Telepathic bond* can be made permanent with a *[[Permanency (Spell)|permanency]]* spell, though it only bonds two creatures per casting of *[[Permanency (Spell)|permanency]]*.

**GM Note**

*House Rule:* In the World of Greyhawk campaign setting this spell is known as *Rary's telepathic bond.*


---
## Tags
#Spell-Resistance/No #Rule-System/Pathfinder #Spell-Range/Close--25-ft----5-ft--2-levels- #Spell-School/Divination #Saving-Throws/None #Spell-Level/Sorcerer-Wizard-5 #Rule/OGL #Sources/PF-Core-Rulebook #Rule/Spell #Rule/Core #Publisher/Paizo #Spell-Rarity/Common #Rule/House-Rule #Rule/fm3

