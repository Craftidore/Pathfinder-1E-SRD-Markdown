---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Aliases:
  - "Undying Aura"
Category: "Spells"
Tags:
  - "Category/Spells"
parent:
  - "U"
up:
  - "U"
prev:
  - "Utterdark (Spell)"
next:
  - "Unbind Chakra (Spell)"
RWtopicId: "Topic_17842"
---
# Undying Aura (Spell)
## Overview
[[Transmutation Spells|Transmutation]]

**Level:** [[5th Level Cleric Spells|Cleric 5]]

**BoXM:** [[BoXM 9th-Level Cleric Spells List|Cleric 9]]

**Casting Time:** 1 standard action

**Components:** V, S, F (a clear spar of crystal worth 500 gp.)

**Range:** Touch

**Target:** Living creature touched

**Duration:** 1 minute/level or until discharged

**Saving Throw:** Will partial

**Spell Resistance:** Yes

**Source:** Magic of Eberron (Wotc), pg. 103

*Undying aura* wreathes the target creature in a golden shield of positive energy, granting it immunity to death effects, energy drain effects, and any negative energy effects (such as from *[[Chill Touch (Spell)|chill touch]]* or an *inflict* spell).

Any time during the spell’s duration, the target can release the power of the aura by making a melee touch attack against an undead creature. On a successful attack, the aura pulses angrily as it tears apart the negative energy sustaining the creature, dealing 1d6 points of damage per caster level (maximum 15d6) and dazing it for 1 round. A successful Will save halves the damage and negates the daze effect. Since the spell utilizes positive energy to deal damage, incorporeal creatures do not benefit from the normal miss chance for incorporeality. Using the touch attack ends the spell’s duration.

A deathless creature (*Eberron Campaign Setting* 275) targeted by the touch attack power of this spell is instead healed of 1d6 points of damage per caster level (maximum 15d6). Some suggest that this indicates the spell was originally designed by the Aereni, and the name of the spell seems to support this claim.


---
## Tags
#Sources/Magic-of-Eberron #Rule-System/Pathfinder #Rule/Spell #Rule/3PP #Publisher/WotC #Spell-Rarity/Rare #Spell-Resistance/Yes #Spell-Level/Cleric-5 #Spell-Range/Touch #Spell-School/Transmutation #Saving-Throws/Will-Partial #Rule/NonOGL

