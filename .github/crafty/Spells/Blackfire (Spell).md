---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Aliases:
  - "Blackfire"
Category: "Spells"
Tags:
  - "Category/Spells"
parent:
  - "B"
up:
  - "B"
prev:
  - "Black Tentacles (Spell)"
next:
  - "Banish Dragonmark (Spell)"
RWtopicId: "Topic_7096"
---
# Blackfire (Spell)
## Overview
[[Necromancy Spells|Necromancy]] \[[[Evil Spells|evil]]]

**Level:** [[8th Level Sorcerer Spells|Sorcerer 8]], [[8th Level Wizard Spells|Wizard 8]]

**BoXM:** [[BoXM 16th-Level Wizard Spells List|Wizard 16]]

**Casting Time:** 1 standard action

**Components:** V, S, M (a pinch of dust from a vampire destroyed by sunlight)

**Range:** Medium (100 ft. + 10 ft./level)

**Effect:** Ray

**Duration:** 1 round/level

**Saving Throw:** Fortitude partial and Reflex negates; see text

**Spell Resistance:** Yes

**Source:** Spell Compendium (WotC), pg. 29

*With the dark words still on your tongue, a ray of black energy springs from your hand to wrap the indicated target in black flames that absorb heat rather than create it. The black fire crackles and hisses, emanating the smells of hot metal and sulfur.*

You must succeed on a ranged touch attack with the ray to strike a target. On a successful attack, a living target is engulfed in chill black flames that feed on the fuel of their victim’s life force. A creature engulfed in *blackfire* must make a successful Fortitude save each round that the spell is in effect or take 1d4 points of Constitution damage and become [[Nauseated|nauseated]]. A creature that makes its Fortitude save takes no damage for that round and is [[Sickened|sickened]] instead. In addition to its effects on the subject, blackfire can spread rapidly. Each round on its turn, any living creature adjacent to a creature engulfed in *blackfire* must succeed on a Reflex save or become engulfed itself.

Any creature that has its Constitution reduced to 0 or lower by the spell is turned into a pile of black ash and can be returned to life only by *[[True Resurrection (Spell)|true resurrection]]* or *[[Wish (Spell)|wish]]*, the caster of which must succeed on a DC 30 caster level check to restore the victim to life. If a creature succeeds on its Fortitude save in 3 consecutive rounds, the *blackfire* affecting it gutters out. The black flames cannot otherwise be extinguished by normal means (such as immersion in water or smothering), but *[[Antimagic Field (Spell)|antimagic field]]*, a successful *[[Dispel Magic (Spell)|dispel magic]]*, *[[Remove Curse (Spell)|remove curse]]*, or *[[Break Enchantment (Spell)|break enchantment]]* snuffs it out. As well, a creature protected by *[[Death Ward (Spell)|death ward]]* has immunity to *blackfire’s* effects.


---
## Tags
#Sources/Spell-Compendium #Rule/Spell #Publisher/WotC #Rule/3PP #Rule/fm3 #Spell-Effect/Ray #Rule/NonOGL #Spell-Rarity/Rare #Saving-Throws/See-text #Spell-Resistance/Yes #Rule-System/Pathfinder #Spell-Descriptor/Evil #Saving-Throws/Fortitude-Partial #Spell-Range/Medium--100-ft----10-ft--level- #Saving-Throws/Reflex-Negates #Spell-Level/Sorcerer-Wizard-8

