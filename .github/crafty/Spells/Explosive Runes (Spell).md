---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Aliases:
  - "Explosive Runes"
Category: "Spells"
Tags:
  - "Category/Spells"
parent:
  - "E"
up:
  - "E"
prev:
  - "Etherealness (Spell)"
next:
  - "Eagle's Form (Spell)"
RWtopicId: "Topic_16062"
---
# Explosive Runes (Spell)
## Overview
[[Abjuration Spells|Abjuration]] \[[[Force Spells|force]]]

**Level:** [[3rd Level Sorcerer Spells|Sorcerer 3]], [[3rd Level Wizard Spells|Wizard 3]]

**BoXM:** [[BoXM 5th-Level Wizard Spells List|Wizard 5]]

**Casting Time:** 1 standard action

**Components:** V, S

**Range:** Touch

**Target:** One touched object weighing no more than 10 lbs.

**Duration:** Permanent until discharged (D)

**Saving Throw:** See text

**Spell Resistance:** Yes

**Source:** PF Core Rulebook, pg. 279

You trace mystic runes upon a book, map, scroll, or similar object  bearing written information. The *explosive runes* detonate when read, dealing 6d6 points of force damage. Anyone next to the *explosive runes* (close enough to read them) takes the full damage with no saving throw; any other creature within 10 feet of the *explosive runes* is entitled to a Reflex save for half damage. The object on which the *explosive runes* were written also takes full damage (no saving throw).

You and any characters you specifically instruct can read the protected writing without triggering the *explosive runes.* Likewise, you can remove the *explosive runes* whenever desired. Another creature can remove them with a successful *[[Dispel Magic (Spell)|dispel magic]]* or *[[Erase (Spell)|erase]]* spell, but attempting to dispel or erase the *explosive runes* and failing to do so triggers the explosion.

Magic traps such as *explosive runes* are hard to detect and disable. A character with the trapfinding class feature (only) can use [[Disable Device (Skill)|Disable Device]] to thwart *explosive runes*. The DC to find magic traps using [[Perception (Skill)|Perception]] and to disable them is 25 + spell level, or 28 for *explosive runes*.


---
## Tags
#Spell-Rarity/Common #Saving-Throws/See-text #Spell-Resistance/Yes #Rule-System/Pathfinder #Spell-School/Abjuration #Spell-Descriptor/Force #Spell-Level/Sorcerer-Wizard-3 #Spell-Range/Touch #Rule/OGL #Sources/PF-Core-Rulebook #Rule/Spell #Publisher/Paizo #Rule/Core

