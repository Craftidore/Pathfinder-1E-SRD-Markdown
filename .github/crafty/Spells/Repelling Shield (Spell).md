---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Aliases:
  - "Repelling Shield"
Category: "Spells"
Tags:
  - "Category/Spells"
parent:
  - "R"
up:
  - "R"
prev:
  - "Rebirth of Iron (Spell)"
next:
  - "Resinous Tar (Spell)"
RWtopicId: "Topic_15486"
---
# Repelling Shield (Spell)
## Overview
[[Abjuration Spells|Abjuration]] \[[[Force Spells|force]]]

**Level:** [[3rd Level Sorcerer Spells|Sorcerer 3]], [[3rd Level Wizard Spells|Wizard 3]]

**BoXM:** [[BoXM 5th-Level Wizard Spells List|Wizard 5]]

**Casting Time:** 1 standard action

**Components:** V, S

**Range:** Personal

**Target:** You

**Duration:** 1 minute/level (D)

**Saving Throw:** None

**Spell Resistance:** No

**Source:** Complete Mage (WotC)­, pg. 115

*You create a shield of force that can push your attackers away from you.*

This spell functions like the *[[Shield (Spell)|shield]]* spell, except that any adjacent creature that makes a melee attack against you is also pushed 5 feet away from you after the attack is resolved. A successful Reflex save negates this effect, and spell resistance applies. You can choose the direction of the creature's movement, as long as it ends up farther away from you than it began. You can't push a creature into a space that it couldn't legally occupy (such as a wall). If no legal space exists to push the creature, it is unaffected.


---
## Tags
#Spell-Resistance/No #Spell-School/Abjuration #Spell-Descriptor/Force #Saving-Throws/None #Spell-Range/Personal #Spell-Level/Sorcerer-Wizard-3 #Rule/NonOGL #Sources/Complete-Mage #Rule/Spell #Rule/3PP #Publisher/WotC #Rule-System/Pathfinder #Spell-Rarity/Rare

