---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Aliases:
  - "Overcompensation"
Category: "Spells"
Tags:
  - "Category/Spells"
parent:
  - "O"
up:
  - "O"
prev:
  - "Orchid's Drop (Spell)"
next:
  - "Old Salt's Curse (Spell)"
RWtopicId: "Topic_17166"
---
# Overcompensation (Spell)
## Overview
[[Transmutation Spells|Transmutation]]

**Level:** [[1st Level Sorcerer Spells|Sorcerer 1]], [[1st Level Wizard Spells|Wizard 1]]

**BoXM:** [[BoXM 1st-Level Wizard Spells List|Wizard 1]]

**Casting Time:** 1 standard action

**Components:** V, S

**Range:** Touch

**Target:** Weapon touched

**Duration:** 1 hour/level

**Saving Throw:** Will negates (object, harmless)

**Spell Resistance:** Yes (object, harmless)

**Source:** 1001 Spells (Rite Publishing), pg. 169

This spell causes the weapon touched to grow to the next smallest size that would normally make it impossible to wield. The damage dealt by the weapon increases as though it were one size category larger, but the weapon can be wielded as though it were its original size. This effect does not stack with other magic effects that increase size, such as *[[Enlarge Person (Spell)|enlarge person]]* or *[[Righteous Might (Spell)|righteous might]]*.


---
## Tags
#Spell-Resistance/Yes #Rule-System/Pathfinder #Rule/OGL #Rule/Spell #Spell-Rarity/Uncommon #Spell-Level/Sorcerer-Wizard-1 #Spell-Range/Touch #Spell-School/Transmutation #Saving-Throws/Will-Negates #Rule/3PP #Publisher/Rite-Publishing #Sources/1001-Spells #Rule/fm3

