---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Aliases:
  - "Miasma"
Category: "Spells"
Tags:
  - "Category/Spells"
parent:
  - "M"
up:
  - "M"
prev:
  - "Metal Melt (Spell)"
next:
  - "Miasma of Entropy (Spell)"
RWtopicId: "Topic_10050"
---
# Miasma (Spell)
## Overview
[[Evocation Spells|Evocation]]

**Level:** [[6th Level Druid Spells|Druid 6]]

**BoXM:** [[BoXM 11th-Level Druid Spells List|Druid 11]]

**Casting Time:** 1 standard action

**Components:** V, S, DF

**Range:** Close (25 ft. + 5 ft./2 levels)

**Target:** One living creature

**Duration:** 3 rounds/level

**Saving Throw:** Fortitude negates; see text

**Spell Resistance:** Yes

**Source:** Spell Compendium (WotC), pg. 141

*You hiss out a red mist that streaks toward your target despite prevailing winds and claws into his mouth and nose, choking him.*

By filling the subject’s mouth and throat with unbreathable gas, you prevent it from doing much more than coughing and spitting. An affected creature cannot speak, cast spells with verbal components, use breath weapons, or utter command words to activate magic items, but it can otherwise act normally.

The subject can hold its breath for 2 rounds per point of Constitution but must make a Constitution check (DC 10 +1 per previous success) each round thereafter to continue doing so. Failure on any such check (or voluntary resumption of breathing) causes the subject to fall unconscious (0 hp). On the next round, the subject drops to –1 hit points and is dying; on the third round, it suffocates.


---
## Tags
#Spell-Resistance/Yes #Sources/Spell-Compendium #Rule-System/Pathfinder #Rule/NonOGL #Rule/Spell #Rule/3PP #Publisher/WotC #Spell-Rarity/Rare #Saving-Throws/See-text #Spell-Range/Close--25-ft----5-ft--2-levels- #Spell-Level/Druid-6 #Spell-School/Evocation #Saving-Throws/Fortitude-Negates

