---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Aliases:
  - "Whispercast"
Category: "Spells"
Tags:
  - "Category/Spells"
parent:
  - "W"
up:
  - "W"
prev:
  - "Whispering Wind (Spell)"
next:
  - "Whirling Weapon (Spell)"
RWtopicId: "Topic_7084"
---
# Whispercast (Spell)
## Overview
[[Transmutation Spells|Transmutation]]

**Level:** [[2nd Level Sorcerer Spells|Sorcerer 2]], [[2nd Level Wizard Spells|Wizard 2]]

**BoXM:** [[Book of Experimental Might Wizard Spell Lists|Wizard 3]]

**Casting Time:** 1 swift action

**Components:** V

**Range:** Personal

**Target:** You

**Duration:** 1 round

**Saving Throw:** None

**Spell Resistance:** No

**Source:** Lords of Madness (WotC)­, pg. 129

The next spell you cast can be cast as a purely mental action, provided that it is of 5th level or lower and that you can whisper. It requires no somatic or material components, unless the spell normally requires a focus or an expensive material component (any component worth more than 1 gp). If the spell does require a focus or an expensive component, you can still *whispercast* it as long as you have the focus or expensive component on your person—the magic of *whispercast* causes the focus to be used or the expensive component to be consumed normally, without you having to handle or manipulate it.

If you do not cast a second spell while *whispercast* is in effect, you gain no benefit from casting *whispercast*.

Casting this spell is a swift action. It does not require a move action or standard action, but you can take only one swift, immediate, or quickened action per turn.

Tsochari spellcasters use this spell to permit spellcasting while they inhabit other creatures. It is also useful for grappled spellcasters.

**GM Note**

*House Rule:* Spells that are *whispercast* can be more difficult to identify via [[Spellcraft (Skill)|Spellcraft]]; spells with a V or S component add an additional +5 penalty to the difficulty to do so.


---
## Tags
#Rule/NonOGL #Rule/House-Rule #Sources/Lords-of-Madness #Rule/Spell #Publisher/WotC #Rule/3PP #Rule/fm3 #Spell-Rarity/Rare #Spell-Resistance/No #Rule-System/Pathfinder #Saving-Throws/None #Spell-Range/Personal #Spell-Level/Sorcerer-Wizard-2 #Spell-School/Transmutation

