---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Aliases:
  - "Imperative Ambulation"
Category: "Spells"
Tags:
  - "Category/Spells"
parent:
  - "I"
up:
  - "I"
prev:
  - "Interminable Echo (Spell)"
next:
  - "Inquisition (Spell)"
RWtopicId: "Topic_15474"
---
# Imperative Ambulation (Spell)
## Overview
[[Enchantment Spells|Enchantment]] ([[Compulsion Spells|compulsion]]) \[[[Mind-Affecting Spells|mind-affecting]]]

**Level:** [[3rd Level Bard Spells|Bard 3]], [[3rd Level Sorcerer Spells|Sorcerer 3]], [[3rd Level Wizard Spells|Wizard 3]] 

**BoXM:** [[BoXM 6th-Level Wizard Spells List|Wizard 6]]

**Casting Time:** 1 standard action

**Components:** V

**Range:** Close (25 ft. + 5 ft./2 levels)

**Target:** One creature

**Duration:** 1 round/3 levels

**Saving Throw:** Will negates

**Spell Resistance:** Yes

**Source:** Complete Mage (WotC), pg. 112

*With seemingly no regard for his safety, your target begins moving around the battlefield.*

For the duration of the spell, the subject must begin each of its turns by taking a move action to move at least 10 feet (provoking attacks of opportunity as normal). The subject has full control over where it moves, as long as it ends up at least 10 feet from where it started. If it is incapable of moving at least 10 feet with a move action—due to terrain or other obstacles—it is [[Dazed|dazed]] for 1 round instead.

**GM Note**

*House Rule:* In the World of Greyhawk campaign setting, this spell commonly known as *Otto's Imperative Ambulation*.

*House Rule:* Added Sorcerer 3, Wizard 3 to Level, per Greytalk forum (ie., “Otto is a wizard.”).


---
## Tags
#Spell-Resistance/Yes #Rule-System/Pathfinder #Spell-Level/Bard-3 #Spell-Range/Close--25-ft----5-ft--2-levels- #Spell-Subschool/Compulsion #Spell-School/Enchantment #Spell-Descriptor/Mind-Affecting #Spell-Level/Sorcerer-Wizard-3 #Saving-Throws/Will-Negates #Rule/NonOGL #Spell-Rarity/Rare #Rule/House-Rule #Sources/Complete-Mage #Rule/Spell #Rule/3PP #Publisher/WotC

