---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Aliases:
  - "Dragonblood Spell-Pact"
Category: "Spells"
Tags:
  - "Category/Spells"
parent:
  - "D"
up:
  - "D"
prev:
  - "Dragonblood Affinity (Spell)"
next:
  - "Dragoncall (Spell)"
RWtopicId: "Topic_7241"
---
# Dragonblood Spell-Pact (Spell)
## Overview
[[Transmutation Spells|Transmutation]]

**Level:** [[5th Level Sorcerer Spells|Sorcerer 5]]

**BoXM:** [[BoXM 9th-Level Wizard Spells List|Wizard 9]]

**Casting Time:** 10 min.

**Components:** V, S, M, XP

**Range:** Touch

**Targets:** You and one willing creature touched

**Duration:** Instantaneous

**Saving Throw:** None

**Spell Resistance:** No

**Source:** Dragons of Faerun (WotC), pg. 115

*Magic swirls around you and your ally, sparkling with the intensity of secret knowledge.*

This spell allows two willing dragonblood creatures to trade known spells. The two creatures agree upon a spell to swap. These spells must be of the same level, though not of the same school. This effect does not grant the creatures the ability to learn more spells. When a spell is traded, it is no longer in the repertoire of the trader. Each participant must expend XP equal to 100 times the minimum caster level necessary to cast the spell.

For instance, Darsikh (an 11th-level half-bronze dragon sorcerer) casts this spell to trade with Arethnektilhimon (an old brass dragon who casts as an 11th-level sorcerer). They agree that Darsikh will give Arethnektilhimon *[[Hallucinatory Terrain (Spell)|hallucinatory terrain]]* in return for *[[Shadow Conjuration (Spell)|shadow conjuration]]*. When the trade is decided and the spell cast, each expends 800 XP (100×8, since 8 is the minimum caster level necessary for sorcerers to cast 4th-level spells). The old spells are no longer in the repertoire of the casters, and are replaced by the spells they have newly chosen.

*Material Component:* Blood; each participant must take 1d4 points of damage when they cut themselves in a blood ceremony to seal their trade.

*XP Cost:* 100 × the minimum caster level necessary to cast the traded spells. Both participants expend this amount.

*Special:* Only dragons and dragonblood creatures can cast this

spell.


---
## Tags
#Sources/Dragons-of-Faerun #Rule/Spell #Rule/3PP #Publisher/WotC #Spell-Rarity/Rare #Rule/NonOGL #Spell-Resistance/No #Rule-System/Pathfinder #Saving-Throws/None #Spell-Level/Sorcerer-Wizard-5 #Spell-Range/Touch #Spell-School/Transmutation

