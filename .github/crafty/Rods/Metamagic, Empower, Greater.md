---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Aliases:
  - "greater rod of empower metamagic"
Category: "Rods"
Tags:
  - "Category/Rods"
parent:
  - "Rods"
up:
  - "Rods"
prev:
  - "Metamagic, Empower, Lesser"
next:
  - "Metamagic, Maximize"
RWtopicId: "Topic_18209"
---
# Metamagic, Empower, Greater
## Overview
**Aura** strong (no school); **CL** 17th

**Slot** none

**Price** 73,000 gp

**Weight** 5 lbs.

**Description**

The wielder can cast up to three spells (from 1st to 9th level) per day that are empowered as though using the [[Empower Spell (Feat)|Empower Spell]] feat.

**Construction**

**Requirements** [[Craft Rod (Feat)|Craft Rod]], [[Empower Spell (Feat)|Empower Spell]]

**Cost** 36,500 gp

**Source:** Core Rulebook, pg. 484


---
## Tags
#Sources/PF-Core-Rulebook #Rule-System/Pathfinder #Magic-Item-Type/Rod #Rule/Core #Rule/Rod #Publisher/Paizo #Price-Band/16-001--gp

