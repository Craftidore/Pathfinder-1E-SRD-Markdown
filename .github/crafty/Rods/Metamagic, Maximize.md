---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Aliases:
  - "rod of maximize metamagic"
Category: "Rods"
Tags:
  - "Category/Rods"
parent:
  - "Rods"
up:
  - "Rods"
prev:
  - "Metamagic, Empower, Greater"
next:
  - "Metamagic, Maximize, Lesser"
RWtopicId: "Topic_18210"
---
# Metamagic, Maximize
## Overview
**Aura** strong (no school); **CL** 17th

**Slot** none

**Price** 54,000 gp

**Weight** 5 lbs.

**Description**

The wielder can cast up to three spells (from 1st to 6th level) per day that are maximized as though using the [[Maximize Spell (Feat)|Maximize Spell]] feat.

**Construction**

**Requirements** [[Craft Rod (Feat)|Craft Rod]], [[Maximize Spell (Feat)|Maximize Spell]] feat

**Cost** 27,000 gp

**Source:** Core Rulebook, pg. 485


---
## Tags
#Sources/PF-Core-Rulebook #Rule-System/Pathfinder #Magic-Item-Type/Rod #Rule/Core #Rule/Rod #Publisher/Paizo #Price-Band/16-001--gp

