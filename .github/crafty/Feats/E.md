---
ImportedOn: "Friday, January 27, 2023 12:02:47 AM"
Category: "Feats"
Tags:
  - "Category/Feats"
parent:
  - "Feat Descriptions"
up:
  - "Feat Descriptions"
prev:
  - "D"
next:
  - "F"
down:
  - "Exploit Adjustment (Feat)"
  - "Easy Metamagic (Feat)"
  - "Enable Critical (Feat)"
  - "Extra Slot (Feat)"
  - "Extra Use (Feat)"
  - "Enhanced Darkvision (Feat)"
  - "Entourage (Feat)"
  - "Extra Spell (Feat)"
  - "Essence Mastery (Feat)"
  - "Eschew Materials (Feat)"
  - "Eidetic Spellcaster (Feat)"
  - "Exceptional Artisan (Feat)"
  - "Extra Rings (Feat)"
  - "Extraordinary Artisan (Feat)"
  - "Elemental Channel (Feat)"
  - "Exhausting Critical (Feat)"
  - "Exotic Weapon Proficiency (Feat)"
  - "Extra Channel (Feat)"
  - "Extra Ki (Feat)"
  - "Extra Lay On Hands (Feat)"
  - "Extra Mercy (Feat)"
  - "Extra Performance (Feat)"
  - "Extra Rage (Feat)"
  - "Expert Aim (Feat)"
  - "Efficient Pull (Feat)"
  - "Elfmarked [Racial] (Feat)"
  - "Elfmarked (Feat)"
  - "Extra Finesse (Feat)"
  - "Extra Rogue Talent (Feat)"
  - "Esoteric Advantage (Feat)"
  - "Emergency Attunement (Feat)"
  - "Endurance (Feat)"
  - "Extend Spell (Feat)"
  - "Enlarge Spell (Feat)"
  - "Empower Spell (Feat)"
  - "Extra Revelation (Feat)"
  - "Extra Hex (Feat)"
  - "Extra Spell Slots (Feat)"
  - "Energy Channel (Feat)"
  - "Esoteric Linguistics (Feat)"
  - "Evolved Companion (Feat)"
  - "Evolved Summoned Monster (Feat)"
  - "Expanded Preparation (Feat)"
  - "Extended Animal Focus (Feat)"
  - "Extra Arcanist Exploit (Feat)"
  - "Extra Inspiration (Feat)"
  - "Extra Investigator Talent (Feat)"
  - "Extra Martial Flexibility (Feat)"
  - "Extra Panache (Feat)"
  - "Extra Reservoir (Feat)"
  - "Extra Slayer Talent (Feat)"
  - "Extreme Prejudice (Feat)"
RWtopicId: "Topic_16105"
---
# E
## Overview
| **Feats** | **Prerequisites** | **Benefits** |
|---|---|---|
| [[Easy Metamagic (Feat)\|Easy Metamagic]] | Any other metamagic feat | Choose a metamagic feat you already have. When paring or casting a spell modified by that feat, lower the spell-slot cost by one. You can never reduce the spell-slot cost below one level higher than the spell's actual level. |
| Ecclesiarch | [[Knowledge (Skill)\|Knowledge]] (religion) 6 ranks | Gather Information and [[Knowledge (Skill)\|Knowledge]] (local) are class skills, +2 bonus on [[Leadership (Feat)\|Leadership]] score |
| Education | — | All [[Knowledge (Skill)\|Knowledge]] skills are class skills, +1 on checks with any two [[Knowledge (Skill)\|Knowledge]] skills |
| Elemental Adept | Elemental mastery class feature | Cast a spell from your chosen element spontaneously |
| Eagle Eyes | Wis 13, keen senses racial trait | Ignore up to –5 due to distance on visual [[Perception (Skill)\|Perception]] checks |
| Eclectic | Human | Gain an additional favored class |
| Efficient Focus Shift | Occultist level 7th | Shift mental focus points without losing any 2/day |
| Efficient Pull\* | Point Blank Shot | Treat your Str as +2 for determining damage with a composite bow |
| Eidetic Spellcaster | Int 17+, wizard level 1st | Record spells in your perfect memory instead of spellbooks |
| Eldritch Claws | Str 15, natural weapons, base attack bonus +6 | Natural weapons treated as magic and silver |
| Eldritch Dragonbane | Ability to invoke an eldritch blast, any true or aberrant dragonmark | Your eldritch blast deals extra damage to dragons and creatures with the dragonblood subtype |
| Eldritch Mark | Ability to invoke an eldritch blast, any true or aberrant dragonmark | Spend 1 action point to render a target flat-footed with your eldritch blast |
| Elemental Channel | Channel energy class feature | Channel energy can harm or heal elementals |
| Elemental Fist\* | Con 13, Wis 13, Improved Unarmed Strike, base attack bonus +8 | Deal 1d6 energy damage with an unarmed strike |
| Elemental Focus | — | +1 bonus on save DCs for one energy type |
| Greater Elemental Focus | Elemental Focus | +1 bonus on save DCs for one energy type |
| Elfmarked | Elfmarked | You count as both human and elf for any effects related to race |
| Elongated Cranium | — | Gain bonuses and penalties based on head shape |
| Elusive Redirection | Elusive target class feature, Combat Expertise, Improved Unarmed Strike, flowing monk level 12th | When you successfully avoid damage, spend 1 ki point to redirect attack back at opponent |
| Elven Accuracy\* | Elf | Reroll miss chance due to concealment when using a bow |
| Emergency Attunement | Spellcraft 7 ranks | Alter one of your ongoing abjuration or transmutation spells |
| Emotional Conduit | Shared consciousness class feature | Gain new spells tied to phantom’s emotional focus |
| Empath | Psychic Sensitivity or ability to cast psychic spells, Perception 1 rank or Sense Motive 1 rank | Read emotions quickly and often, but open your mind to risk |
| Enable Critical | Weapon Finesse or Weapon Focus | Choose one type of creature that is normally immune to critical hits (construct, elemental, ooze, plant, undead); you can now score critical hits against that type of creature |
| Endurance | — | +4 bonus on checks to avoid nonlethal damage |
| Energy Abjuration | Spell Focus (abjuration) or abjurer level 1st | Gain energy resistance when casting abjuration spell |
| Energy Channel | Channel energy class feature; Air, Earth, Fire, or Water domain or blessing | Spend channel energy to deal extra energy damage with weapon attacks |
| Energy Gestalt | Spell Focus (evocation), CL 3rd | Gain benefits when casting energy-based spells in succession |
| Enforcer\* | Intimidate 1 rank | Demoralize opponent as free action when you inflict nonlethal damage |
| Enhanced Darkvision | Wis 15+, Alertness, darkvision 60 ft. or more | Your darkvision range is doubled |
| Entourage | Cha 17+, Leadership | Take Leadership feat multiple times |
| Entreating Critical\* | Call Truce, Critical Focus, base attack bonus +11 | Use a critical hit to convince foes to agree to a truce |
| Eschew Materials | — | Cast spells without material components |
| Esoteric Advantage | Knowledge (arcana, dungeoneering, local, nature, planes, or religion) 3 ranks | After identifying an individual creature with a successful Knowledge skill check, you can spend a move action to adapt your tactics to reduce that individual’s natural defenses |
| Esoteric Linguistics | Skill Focus (Linguistics) | Use Linguistics in place of some Use Magic Device and Spellcraft checks |
| Essence Mastery | No ability score below 10, 15+ ranks in Knowledge (arcana) skill, 8+ ranks in Knowledge (nature), 8+ ranks in Knowledge (religion), 15+ ranks in Spellcraft skill, any three metamagic feats, non-specialist wizard only | When learning a new spell, you may draw from any spell list, arcane, divine, or otherwise |
| Evil Brand | — | +2 bonus on Diplomacy and Intimidate checks made against evil creatures |
| Evil Embraced | Evil Brand | Gain damage reduction 10/good for 1 round |
| Evoker’s Mark | Spell Focus (evocation), any true or aberrant dragonmark | Spend 1 action point to empower an evocation spell |
| Evolved Companion | Cha 13, animal companion class feature | Companion gains a 1-point eidolon evolution |
| Evolved Summoned Monster | Augment Summoning, Spell Focus (conjuration), ability to cast *summon monster I* | Summoned monster gains a 1-point eidolon evolution |
| Exhausting Critical\* | Tiring Critical, base attack bonus +15 | Whenever you score a critical hit, the target is exhausted |
| Exorcising Mutilation | Con 15, Clarity of Pain | Grievously damage yourself to fight off a dominate effect or possession |
| Exotic Weapon Proficiency\* | Base attack bonus +1 | No penalty on attacks made with one exotic weapon |
| Expanded Arcana | Caster level 1st, see feat | Add one or two spells to list of spells known |
| Expanded Phrenic Pool | Phrenic pool class feature | Gain 2 additional phrenic pool points |
| Expanded Preparation | Arcanist level 1st | Gain extra spell slots |
| Expeditious Sleuth | Perception 3 ranks, inspiration or lore master class feature | Search faster and gain a +2 bonus when you take 20 |
| Expert Aim | Precise Shot, base attack bonus +4 | Take a single attack at a target within 30 ft, add Dex modifier to damage |
| Expert Driver | Skilled Driver | Maneuver a vehicle of a selected type as a move action, and you can stop immediately |
| Exquisite Sneak | Dex 15, Careful Sneak, Stealth 6 ranks | Use Stealth without penalty from most armor |
| Extend Rage | Ability to rage or frenzy | Add 5 rounds to duration of your rage |
| Extend Resonant Power | Caster level 7th, mental focus class feature | Share resonant powers with an ally |
| Extended Animal Focus | Animal focus class feature | Add your Wisdom modifier to the number of minutes per day you can use animal focus |
| Extra Amplification | Phrenic amplification class feature | Gain one additional phrenic amplification |
| Extra Arcanist Exploit | Arcanist exploit class feature | Gain an additional arcanist exploit |
| Extended Stare | Hypnotic stare class feature | Increase hypnotic stare’s range by 10 feet |
| Extra Bane | Bane class feature | May use bane ability 3 additional times per day |
| Extra Bombs | Bomb class feature | Throw two additional bombs per day |
| Extra Channel | Channel energy class feature | Channel energy two additional times per day |
| Extra Contingency | Character level 19th | Have two active contingency effects |
| Extra Discovery | Discovery class feature | Gain one additional discovery |
| Extra Finesse | Weapon Finesse | Use Dex instead of Str on damage rolls with light weapons |
| Extra Focus Power | Focus power class feature | Gain one additional focus power |
| Extra Hex | Hex class feature | Gain one additional hex |
| Extra Inspiration | Amateur Investigator or inspiration class feature | Gain three uses of inspiration |
| Extra Investigator Talent | Investigator talent class feature | Gain an additional investigator talent |
| Extra Ki | Ki pool class feature | Increase your ki pool by 2 points |
| Extra Lay On Hands | Lay on hands class feature | Use lay on hands two additional times per day |
| Extra Martial Flexibility | Martial flexibility class feature | Gain three uses of martial flexibility |
| Extra Mental Focus | Mental focus class feature | Gain 2 additional points of mental focus |
| Extra Mercy | Mercy class feature | Your lay on hands benefits from one additional mercy |
| Extra Mesmerist Tricks | Mesmerist trick class feature | Implant two additional tricks per day |
| Extra Music | Bardic music | Use bardic music four extra times per day |
| Extra Panache | Amateur Swashbuckler or panache class feature | Gain 2 extra panache points |
| Extra Performance | Bardic performance class feature | Use bardic performance for 6 additional rounds per day |
| Extra Rage | Rage class feature | Use rage for 6 additional rounds per day |
| Extra Rage Power | Rage power class feature | Gain one additional rage power |
| Extra Reservoir | Arcane reservoir class feature | Gain 3 more points in your arcane reservoir |
| Extra Revelation | Revelation class feature | Gain one additional revelation |
| Extra Rogue Talent | Rogue talent class feature | Gain one additional rogue talent |
| Extra Slayer Talent | Slayer talent class feature | Gain an additional slayer talent |
| Extra Slot | Caster level 4th | Gain an extra spell slot up to one level lower than current highest level |
| Extra Spell | Int 13+, Wis 13+, Wizard leveI 3rd+ | Prepare one extra spell of any level you can currently cast |
| Extra Spell Slots | Spellcasting class level 3 | Gain two additional spells slots for one spellcasting class you have levels in. You may never use this feat to have more spell slots at any level than you know at each level below it. |
| Extra Touch Treatment | Touch treatment class feature | Use touch treatment two additional times per day |
| Extra use | — | Use any class ability an extra 1/day |
| Extra Wild Talent | Kineticist level 6th | Gain a wild talent at least 2 spell levels lower than maximum |
| Extreme Prejudice\* | Seething Hatred, sneak attack +3d6, studied target class feature | When you succeed at a sneak attack with Seething Hatred, roll d8s for damage instead of d6s |
| Eye for Ingredients | Eschew Materials, Appraise 6 ranks, Spellcraft 6 ranks | Save money buying material components |
| Eye of Medani | Any Mark of Detection | Reroll one Perception check 1/day; reduce miss chances |

  \* This is a combat feat, and can be selected as a brawler, fighter, gunslinger, swashbuckler, and warpriest bonus feat.

---
## Governed Content
- [[Easy Metamagic (Feat)]]
- [[Efficient Pull (Feat)]]
- [[Eidetic Spellcaster (Feat)]]
- [[Elemental Channel (Feat)]]
- [[Elfmarked (Feat)]]
- [[Elfmarked [Racial] (Feat)]]
- [[Emergency Attunement (Feat)]]
- [[Empower Spell (Feat)]]
- [[Enable Critical (Feat)]]
- [[Endurance (Feat)]]
- [[Energy Channel (Feat)]]
- [[Enhanced Darkvision (Feat)]]
- [[Enlarge Spell (Feat)]]
- [[Entourage (Feat)]]
- [[Eschew Materials (Feat)]]
- [[Esoteric Advantage (Feat)]]
- [[Esoteric Linguistics (Feat)]]
- [[Essence Mastery (Feat)]]
- [[Evolved Companion (Feat)]]
- [[Evolved Summoned Monster (Feat)]]
- [[Exceptional Artisan (Feat)]]
- [[Exhausting Critical (Feat)]]
- [[Exotic Weapon Proficiency (Feat)]]
- [[Expanded Preparation (Feat)]]
- [[Expert Aim (Feat)]]
- [[Exploit Adjustment (Feat)]]
- [[Extend Spell (Feat)]]
- [[Extended Animal Focus (Feat)]]
- [[Extra Arcanist Exploit (Feat)]]
- [[Extra Channel (Feat)]]
- [[Extra Finesse (Feat)]]
- [[Extra Hex (Feat)]]
- [[Extra Inspiration (Feat)]]
- [[Extra Investigator Talent (Feat)]]
- [[Extra Ki (Feat)]]
- [[Extra Lay On Hands (Feat)]]
- [[Extra Martial Flexibility (Feat)]]
- [[Extra Mercy (Feat)]]
- [[Extra Panache (Feat)]]
- [[Extra Performance (Feat)]]
- [[Extra Rage (Feat)]]
- [[Extra Reservoir (Feat)]]
- [[Extra Revelation (Feat)]]
- [[Extra Rings (Feat)]]
- [[Extra Rogue Talent (Feat)]]
- [[Extra Slayer Talent (Feat)]]
- [[Extra Slot (Feat)]]
- [[Extra Spell (Feat)]]
- [[Extra Spell Slots (Feat)]]
- [[Extra Use (Feat)]]
- [[Extraordinary Artisan (Feat)]]
- [[Extreme Prejudice (Feat)]]

